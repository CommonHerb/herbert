...
| --- | --- | --- | --- | --- |
| `bootstrap/lex.c` | Tokenize source, comments, literals, operators, and line-sensitive diagnostics. | `stack/lexer_fragment.herb`, copied lexer sections in parser/evaluator/compiler fragments, `stack/error_probes/*.herb`. | `bootstrap/tests/run_tests.sh` drives `stack/lexer_probe` and error probes through the C bootstrap. | A dual-run lexer oracle that feeds the same source corpus to C lexing and Herbert lexing, compares token kinds/text/lines, and includes malformed inputs. |
...
Start with lexer equivalence. It is the smallest surface with a clear
Herbert-owned counterpart and existing probes, but it still needs malformed
input coverage and line-number parity before any C lexer code can be retired.

Candidate first test:

- Build a tiny fixture runner that records C lexer output for accepted source
  snippets.
- Run `stack/lexer_fragment.herb` over the same snippets.
- Compare token kind and text exactly.
...
the minimum harness code needed to make lexer equivalence visible. No bootstrap
file should be deleted in that slice.
