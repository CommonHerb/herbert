...
| --- | --- | --- | --- | --- |
| `bootstrap/lex.c` | Tokenize source, comments, literals, operators, and line-sensitive diagnostics. | `stack/lexer_fragment.herb`, `stack/lexer_stdin_driver.herb`, `stack/lexer_error_driver.herb`, copied lexer sections in parser/evaluator/compiler fragments, `stack/error_probes/*.herb`. | `bootstrap/tests/run_tests.sh` drives `stack/lexer_probe`; `bootstrap/tests/run_lexer_equivalence.sh` normalizes C `lex()` output for an accepted-source corpus, including a focused native/operator-surface fixture, and diffs it against the Herbert stdin lexer driver, then checks the existing `lex_10x` malformed probes against the Herbert lexer diagnostic driver for ERR code, line, and message parity; `bootstrap/tests/check_lexer_copy_sync.py` guards accepted-token and line-aware lexer copies against drift; the full error-probe battery still runs through C bootstrap and Klondike. | Keep growing accepted-source fixture coverage around real parser/compiler/native source shapes before considering any C lexer deletion. |
...
oracle now covers a small accepted-source corpus including native operator
syntax, lexer ERR code, line, and message parity for the existing malformed
lexical probes, plus synchronization coverage for accepted-token and line-aware
lexer copies. It still needs broader accepted-source coverage before any C
lexer code can be retired.
...
- Start the parser replacement proof with a C-AST-to-Herbert-AST comparison
  only after lexer fixture coverage is broad enough to stop hiding token drift.
