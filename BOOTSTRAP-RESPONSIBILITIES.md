...
| --- | --- | --- | --- | --- |
| `bootstrap/lex.c` | Tokenize source, comments, literals, operators, and line-sensitive diagnostics. | `stack/lexer_fragment.herb`, `stack/lexer_stdin_driver.herb`, `stack/lexer_error_driver.herb`, copied lexer sections in parser/evaluator/compiler fragments, `stack/error_probes/*.herb`. | `bootstrap/tests/run_tests.sh` drives `stack/lexer_probe`; `bootstrap/tests/run_lexer_equivalence.sh` normalizes C `lex()` output for an accepted-source corpus and diffs it against the Herbert stdin lexer driver, then checks the existing `lex_10x` malformed probes against the Herbert lexer diagnostic driver for ERR code, line, and message parity; `bootstrap/tests/check_lexer_copy_sync.py` guards accepted-token and line-aware lexer copies against drift; the full error-probe battery still runs through C bootstrap and Klondike. | Grow accepted-source equivalence coverage around native/compiler lexical constructs before considering any C lexer deletion. |
...
message parity for the existing malformed lexical probes, plus synchronization
coverage for accepted-token and line-aware lexer copies. It still needs broader
accepted-source coverage before any C lexer code can be retired.
...
- Add focused fixtures for native/compiler lexical constructs that have
  previously drifted across copied lexer bodies.
