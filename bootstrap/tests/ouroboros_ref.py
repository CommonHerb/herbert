...
def grade_overflow(stream, kend_elf, fed):
    ...
    # The terminal answer is a decoded guest record. G.parse consumes the whole
    # guest tail and rejects duplicate/conflicting answers, while permitting the
    # recognized Bochs host trailer after it. Payload byte windows are opaque.
    # Keep this pin on the guest's final record, not the transport's final bytes.
    terminal = r['_tail'].records[-1]
    if terminal.kind != 'answer':
        errs.append('guest trace does not END with the terminal DE<answer>AD record')
    elif r.get('answer') != terminal.raw[1]:
        errs.append(f'parser answer {_hx(r.get("answer"))} != terminal answer 0x{terminal.raw[1]:x}')
