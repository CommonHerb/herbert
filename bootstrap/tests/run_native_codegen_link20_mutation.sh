...
# anchor and re-emitting a green probe through the C-interpreted backend: each
# mutation must be CAUGHT (a wrong/absent byte under QEMU, a broken page-table hole,
# or a changed structural head).
#
...
emit_seq=0
# assess(compiler_src) -> "GREEN" (valid image: exact head + single page-table hole at
# the neighbor + boot emits the golden byte) or "CAUGHT:<why>".
assess() {
    ...
    local head; head=$(dd if="$d/a.out" bs=1 skip=4108 count=118 status=none 2>/dev/null | xxd -p | tr -d '\n')
...
# control: unmutated compiler must pass head + hole + boot.
ctrl=$(assess "$backend")
if [[ "$ctrl" == "GREEN" ]]; then echo "PASS control: unmutated compiler emits golden=$GOLDEN via the #PF fault path"; pass=$((pass+1));
...
echo "PASS: link20 mutation proof ($pass checks: control passes head+hole+boot gates; 6 mutations each CAUGHT -- page-table-hole-off-neighbor and CR0.PG-not-enabled fall to the 0xBB sentinel; the IDTR-limit shrink triple-faults (no byte); the CR2 and faulting-EIP wrapper-check corruptions fall to the sentinel; and the boot-INVISIBLE CR4-clear neutering is caught by the exact-head template gate, which pins straight-line paging-enable -> unmapped-neighbor touch -> #PF wrapper)"
exit 0
