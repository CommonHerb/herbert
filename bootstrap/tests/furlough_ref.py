#!/usr/bin/env python3
# furlough_ref.py -- STEP-0 oracle + BYTE-EXACT emitter target for "BLOCKING SYS_READ (block / wake)"
# (native-codegen Link 49 / kernel-arc link 33). BLOCK/WAKE: a ring-3 program that issues SYS_READ on input that is
# not yet available is DESCHEDULED by the kernel (parked, consuming ZERO quanta) instead of busy-polling COM1 with
# IF=0. Peers keep running. When the awaited byte arrives, the kernel WAKES the parked reader, delivers the byte, and
# the reader resumes after its int 0x30. This removes a CONFIRMED LIVENESS DEFECT of the frozen lineage: do_read polls
# COM1 inside a DPL-3 INTERRUPT gate (IF=0) at CPL0, and the timer handler skips CPL0 ticks (test cs,3; jz no-switch),
# so a kernel busy-poll is UN-PREEMPTIBLE BY DESIGN -- a naive reader on not-ready input FREEZES the whole machine,
# starving all peers. `sti`/trap-gate cannot fix it (a CPL0 tick still does not switch); the ONLY fix-class for a naive
# (non-yielding) reader is KERNEL-SIDE DESCHEDULE. (Cross-checked: same-model judge panel 5/5 + 5 refuters + Codex.)
#
# Built additively on the FROZEN homestead lineage (furlough_ref started as a byte-identical copy of homestead_ref):
# lodger parse/bump-alloc + nokta paging-U/S + geeking fault->continue + tickover preemptive full-context switch +
# rollcall runtime-K table + tenement region reclaim + homestead demand-paged grow. NEW vs homestead:
#   blocked[]  a new per-proc TCB state array (1 = parked on a not-ready SYS_READ). The scheduler pick loop SKIPS
#              blocked[i]!=0 procs (mirrors the exited[]/alloc_lo==0 skips).
#   disp[]     a new per-proc DISPATCH counter (incremented when sched_switch actually dispatches a proc), dumped at
#              finalize (CA <count_0>..<count_{K-1}> CB). The forge-killer: a genuine block parks the reader so it is
#              dispatched EXACTLY TWICE (fresh-start -> blocks; wake -> completes); a "runnable-retry" forge (deschedule
#              but keep the reader runnable / restart the syscall) re-dispatches it every cycle -> disp[reader] >> 2.
#   BLOCK arm  in do_read: poll COM1 LSR ONCE; if not-ready, snapshot the int-0x30 return frame into TCB[cur]
#              (t_eip=[esp], t_eflags=[esp+8], t_esp=useresp=[esp+0xC]), set blocked[cur]=1, jmp sched_switch.
#   WAKE arm   in sched_switch (before the pick): re-poll COM1 LSR; if a byte is ready AND a proc is blocked, read RBR,
#              inject the byte into t_eax[wakee], clear blocked[wakee]=0, emit a WAKE WITNESS (CC <wakee> <byte> CD).
#   IDLE       if the pick finds NO runnable proc (all blocked/exited), loop back to the wake re-poll (idle until the
#              byte arrives and wakes someone) -- else the pick loop would spin forever (a new freeze; Codex-flagged).
# FORCING (hand-assembled, byte-pinned): THREE programs. A (proc0) = a NAIVE reader: one `mov eax,0; int 0x30`
# (SYS_READ, no retry loop -- SYS_YIELD was deleted at tickover, so no module-side escape), then emits le32(0xAB0000|byte)
# and SYS_EXIT -- output DEPENDS on the delivered byte. B,C (procs 1,2) = token emitters (distinct held-back tokens,
# no SYS_READ) that prove peer-progress. Two graded runs: RUN-1 (A's byte withheld forever) -- frozen kernel FREEZES,
# B,C absent -> RED; furlough parks A, B,C emit -> GREEN (the freeze differential). RUN-2 (byte delivered) -- A wakes,
# emits le32(0xAB0000|byte), disp[A]==2, one wake witness (byte matches), finalize -> GREEN; mutations bite here.
...
# ---- furlough (block/wake) forcing constants ----
...
if __name__=='__main__':
    ...
    else: raise SystemExit('usage: kernelelf|kend|modtiny|modspinner|modworker|modworker_ten|modgrower|modreader|modpeer|modhostile|modhostileread|grade|gradehead|gradeten|gradehome|gradefurl|gradehostile|rollcall|tenement|homestead|furlough|ncells')
