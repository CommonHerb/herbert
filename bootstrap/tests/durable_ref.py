#!/usr/bin/env python3
# durable_ref.py -- reference builder + grader for "durable" (native-codegen link 54 / kernel-arc link 38); this link's CONTRACT is bootstrap/tests/run_native_codegen_link54.sh (+ its _mutation.sh). HEADER ANCESTRY (blind-audit A3, 2026-08-29): this file was cloned from lethe_ref.py and the descriptive block that follows is the ANCESTOR's -- kept as provenance, NOT as this file's spec. Ancestor header:
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'durable', cd['nprocs'])
...
if __name__=='__main__':
    cmd=sys.argv[1]
    if cmd=='durwriter':       # out [seed]  (BOOT-1 writer prober; seed baked if given, else late-bound SYS_READ over COM1)
