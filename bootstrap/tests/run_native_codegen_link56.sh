...
if have_qemu; then
    ...
    for SUB in tcg kvm; do
        ...
        if [[ "$SUB" == kvm ]] && ! have_kvm; then
            # KVM is the LOCAL real-silicon pre-push leg, NOT a CI substrate: GitHub runners have no /dev/kvm. The far-axis
            # CI oracle is QEMU-TCG + Bochs (both REQUIRE_EMU fail-closed); KVM runs when /dev/kvm is present and is SKIPPED
            # (with a note) when absent -- matching EVERY prior link's gate (cairn L55 etc.). NOT fail-closed: KVM is
            # genuinely absent on the runner and present locally for the pre-push real-silicon check, so a hard fail here
            # would break CI for an optional substrate (the [[feedback_ci_only_path_vs_local]] trap -- it passed locally
            # where /dev/kvm exists). REQUIRE_EMU still hard-gates the two substrates that MUST be present (QEMU + Bochs).
...
BX
    ...
    # harness detectors from the link60 reference so a harness failure re-rolls instead of ever grading a confounded
...
PY
    ...
    elif echo "$bochs_emit" | grep -q '^GREEN'; then ok "(C-Bochs) the late-bound alloc/free witness on the EMITTED kernel is GREEN on the 2nd substrate: the kernel-emitted trace == the host first-fit golden (split + coalesce + non-MRU reuse) on Bochs' chipset"
    else fail_test "(C-Bochs) Bochs witness not GREEN (feeder SENT + ran through shutdown; guest RECEIPT unproven feeder-side -- a lone RED may be a capture-class flake, re-derive per the parley replay discriminator): $(echo "$bochs_emit" | tr '\n' ';')"; fi
else
