...
# cross-check -- DEFAULT-ON when C is present, opt-OUT via LINK26_MUTATION_NO_C=1 --
...
XCHECK=0
if [[ -x "$HERBERT" && "${LINK26_MUTATION_NO_C:-0}" != "1" ]]; then XCHECK=1; fi
...
assess() { # compiler ELF -> GREEN | CAUGHT:<why>
    ...
    if [[ -n "$gt_pos" ]]; then
        body_end=$(( gt_pos / 2 ))
        dd if="$EMIT_IMG" bs=1 skip=$(( 4108 + 61 )) count=$(( body_end - 61 )) status=none 2>/dev/null > "$d/body"
    ...
    timeout 30 qemu-system-x86_64 -kernel "$EMIT_IMG" -debugcon file:"$d/e9" -display none \
        -no-reboot -serial none -monitor none -device isa-debug-exit,iobase=0xf4,iosize=0x04 -cpu qemu64 -m 64M >/dev/null 2>"$d/e9.qerr"
...
# emit the golden byte via a genuine 64-bit COMPILED body C-FREE.
ctrl=$(assess "$SEED")
if [[ "$ctrl" == "GREEN" ]]; then echo "PASS control: unmutated seed compiler emits golden=$GOLDEN via a genuine 64-bit COMPILED body C-free"; pass=$((pass+1));
