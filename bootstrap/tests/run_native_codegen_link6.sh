...
check_runtime_parity() {
    ...
    if [[ $c_rc -eq 0 || $n_rc -eq 0 ]]; then
        fail_test "runtime $label expected both nonzero (C rc=$c_rc native rc=$n_rc)"
        return
    fi
    pass=$((pass + 1))
}
...
if [[ $gate_ok -eq 1 ]]; then grep -q '4c 8d b8 00 00 00 01' "$hex" || { fail_test "disasm gate no r15 heap limit"; gate_ok=0; }; fi
