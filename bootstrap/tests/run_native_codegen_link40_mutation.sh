...
qemu_feed() { # kelf mod out fedbyte
    ...
    grep -q LISTENING "$d/feed.log" 2>/dev/null || { echo "FAIL: link40 harness failure -- feeder never reached LISTENING (socket/QEMU launch dead; NOT a mutation bite)" >&2; : > "$HVMARK"; kill "$fp" 2>/dev/null; wait "$fp" 2>/dev/null; return; }
    timeout 60 qemu-system-x86_64 -kernel "$kelf" -initrd "$mod" -debugcon file:"$out" \
        ...
        grep -qvE 'terminating on signal' "$out.qerr" 2>/dev/null && { echo "FAIL: link40 harness failure -- QEMU launch error: $(grep -vE 'terminating on signal' "$out.qerr" | head -1)" >&2; : > "$HVMARK"; }   # F2a: only a NON-timeout stderr line is a launch failure; a timeout-kill (hang bite) is left to the grader
    wait "$fp" 2>/dev/null
}
qemu_noin() { # kelf mod out
    timeout 60 qemu-system-x86_64 -kernel "$1" -initrd "$2" -debugcon file:"$3" \
        -device isa-debug-exit,iobase=0xf4,iosize=0x04 -no-reboot -display none -cpu qemu64 -serial null -monitor none -m 64M >/dev/null 2>"$3.qerr"
    # fail-closed: a QEMU LAUNCH failure writes to stderr, while a clean run -- even a guest fault -- leaves
    # stderr EMPTY. Non-empty stderr is an unambiguous HARNESS failure (an empty debugcon scoring the negative
    # leak/write legs), NOT a bite. (rc is NOT usable: isa-debug-exit yields odd codes >124 on legit runs.)
    grep -qvE 'terminating on signal' "$3.qerr" 2>/dev/null && { echo "FAIL: link40 harness failure -- QEMU launch error: $(grep -vE 'terminating on signal' "$3.qerr" | head -1)" >&2; : > "$HVMARK"; }   # only a NON-timeout stderr line is a launch failure; a timeout-kill (hang bite) is left to the grader
}
...
echo "native-codegen link40 mutation: pass=$pass fail=$fail"
[[ "$fail" -eq 0 ]] || exit 1
if [[ -e "$HVMARK" ]]; then echo "FAIL: link40 HARNESS FAILURE -- a feeder never reached LISTENING (dead socket/QEMU); fail-closed, NOT a genuine pass"; rm -f "$HVMARK"; exit 1; fi
