#!/usr/bin/env bash
#
# Evaluator native-execution gate (sovereignty axis, Role-C reduction).
#
# The metacircular EVALUATOR fragment (stack/evaluator_fragment.herb -- a full
# lex -> parse -> eval -> serialize over the embedded evaluator_probe, the named
# Herbert replacement for the two largest Role-C C files, bootstrap/eval.c +
...
#
# This is the FIRST metacircular fragment to gain a native execution path. Every
# fragment (lexer/parser/evaluator/vm/klondike) has, until now, run ONLY under
# the C interpreter, so deleting C would kill the entire self-description test
# suite. This gate makes the evaluator's self-description test survive C's
# deletion -- it is a genuine Role-C C-removal, not a measurement.
#
...
#       oracle (stack/evaluator_probe.expected). This consults NO C: it runs the
#       gen-1-compiled evaluator and diffs against a committed answer key. It must
#       pass AFTER the C interpreter is deleted. This is the sovereignty advance.
#
#   (2) RETIREABLE / OPTIONAL -- native ELF line 1 == the C interpreter's line 1
#       (a faithfulness cross-check: the native-compiled evaluator agrees with the
...
#       runs only while a C interpreter exists. Set EVALUATOR_NATIVE_NO_C=1 (or
...
# before this link; mutating a reachable evaluator rule makes the native output
# diverge from the oracle (proven by run_evaluator_native_mutation.sh).
#
# SCOPE (honest): the native evaluator is exercised over ONE fixed forcing probe
# (the 12-feature evaluator_probe). That probe is broad -- arithmetic+wrap, all
# six comparisons, short-circuit, non-tail + tail recursion, strings, buffers,
# nested tuples, arrays-of-tuples, reference semantics/aliasing, block scope --
# but "this probe runs natively and matches the oracle" is NOT "the evaluator
# fragment fully replaces eval.c/value.c over all programs." Broader native
# coverage (more programs; the vm/klondike fragments) is a later rung. The main
# entry point is a thin I/O adapter (the result is emitted via flogger and main
# returns 0, because the native subset requires main to return int/bool); the
# 1550 lines of evaluator logic are byte-identical to the interpreted fragment.
set -u
...
fail() { echo "FAIL: evaluator native execution ($1)"; exit 1; }
...
# Runs ONLY while a C interpreter exists. EVALUATOR_NATIVE_NO_C=1 simulates the
...
if [[ "${EVALUATOR_NATIVE_NO_C:-0}" != "1" && -x "$HERBERT" ]]; then
    ci="$tmp/cinterp.out"; ciln="$tmp/cinterp.line1"
    "$HERBERT" "$fragment" >"$ci" 2>/dev/null || fail "C interpreter could not run the evaluator fragment"
    head -1 "$ci" >"$ciln"
    cmp -s "$ciln" "$oracle" || fail "C-interpreted evaluator line 1 differs from the oracle (the fragment regressed under C)"
    cmp -s "$nat" "$ciln" || fail "native gen-1 evaluator diverges from the C interpreter (faithfulness)"
    c_checked="(faithfulness vs C interpreter: native == C-interp == oracle)"
fi

echo "PASS: evaluator native execution (gen-1-compiled evaluator_fragment runs C-FREE, line 1 == independent oracle; $c_checked)"
