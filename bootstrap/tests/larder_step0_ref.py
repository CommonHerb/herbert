#!/usr/bin/env python3
# larder_step0_ref.py -- STEP-0 spike builder for larder's first-fit allocator (validated standalone, then PORTED into larder_ref.py; no gate of its own -- see larder_ref.py / run_native_codegen_link56.sh). HEADER ANCESTRY (blind-audit A3, 2026-08-29): cloned from lethe_ref.py; the descriptive block that follows is the ANCESTOR's -- provenance, not spec. Ancestor header:
...
SCALARS=['mbinfo','flags','cmdline','elflo','elfhi','mm_lo','mm_hi','region_lo','region_hi',
         ...
         'pool_base','pool_size','nchunks','al_size','al_ptr','al_i','al_k','al_ret']  # link40 (larder): heap pool base/size, free-list length, + alloc/free/dump scratch
...
def build_code(kstack, kend, mut=None, stage='full'):
    ...
    def emit_larder_split():
        ...
        a.raw(0x8B,0x07); a.raw(0x03,0x05); a.blob(le32(cell('al_size'))); a.raw(0x89,0x47,0x0C)  # [i+1].base = base_i + size
        a.raw(0x8B,0x47,0x04); a.raw(0x2B,0x05); a.blob(le32(cell('al_size'))); a.raw(0x89,0x47,0x10) # [i+1].len = len_i - size
    ...
    def emit_larder_delete():
        ...
        a.raw(0x89,0xF7); a.raw(0x83,0xEF,0x0C)           # mov edi,esi ; sub edi,12  (edi = &chunk[k] dest)
    ...
    # ===== link40 (larder): carve the heap pool from proc0's User region + init the free-list (ONE big free chunk) =====
    # The pool reuses the LOW end of proc0's already-User-mapped region page (alloc_lo[0]); the module's stack lives at the
    # HIGH end (esp=alloc_hi[0], grows down), so a 168B pool at the bottom and a <128B module scratch at the top never
    # collide. Data spans are thus User (CPL3 r/w) and identity-mapped (CPL0-readable for the kernel readback); the
    # free-list metadata stays in the Supervisor cells above. (STEP-0 simplification -- the real link should carve a
    # DEDICATED pool page via the lodger bump-scan; see the report. The mechanism validated here is identical.)
    mem(arr('alloc_lo')); mme(cell('pool_base'))                      # pool_base = alloc_lo[0]
    ...
    a.lbl('do_alloc')
    load_kdata()
    a.raw(0x89,0x1D); a.blob(le32(cell('al_size')))       # mov [al_size], ebx
    a.raw(0x31,0xC9)                                       # xor ecx,ecx  (i=0)
    a.lbl('la_find')
    ...
    a.j(None,'la_emit')
    a.lbl('la_oom')
    a.raw(0xC7,0x05); a.blob(le32(cell('al_ret'))); a.blob(le32(0))   # al_ret = 0 (NULL on OOM)
    a.lbl('la_emit')
    outi(0xE0)                                            # marker
    ...
    a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))          # mov [al_i], ecx
    ch_esi_from_ecx()
    a.raw(0x8B,0x06); a.raw(0x3B,0x05); a.blob(le32(cell('al_ptr'))); a.j(JNE,'lf_next')   # base!=ptr -> next
    a.raw(0x8B,0x46,0x08); a.raw(0x85,0xC0); a.j(JE,'lf_next')           # already free -> next (defensive)
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'larder_step0', cd['nprocs'])
