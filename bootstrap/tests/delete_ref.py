#!/usr/bin/env python3
# delete_ref.py -- reference builder + grader for "delete" (native-codegen link 58 / kernel-arc link 42); this link's CONTRACT is bootstrap/tests/run_native_codegen_link58.sh (+ its _mutation.sh). HEADER ANCESTRY (blind-audit A3, 2026-08-29): this file was cloned from larder_ref.py and the descriptive block that follows is the ANCESTOR's -- kept as provenance, NOT as this file's spec. Ancestor header:
...
def build_code(kstack, kend, mut=None, stage='full', npages=1, fsdel=False):
...
def build_elf(mut=None, stage='full', npages=1, fsdel=False):   # cleave L847-859 + growheap npages + delete fsdel (fsdel=False == growheap byte-for-byte)
    code0,_=build_code(0,0,mut,stage,npages,fsdel); clen=len(code0)
    memsz=12+clen+16384
    kstack=LOAD+memsz; kend=LOAD+memsz
    code,labels=build_code(kstack,kend,mut,stage,npages,fsdel); assert len(code)==clen,(len(code),clen)
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'delete', cd['nprocs'])
...
if __name__=='__main__':
    cmd=sys.argv[1]
    if cmd=='deletekernel':    # out [mut]  (build the DELETE kernel ELF: fsdel=True, npages=GROWHEAP_NPAGES; print kend)
