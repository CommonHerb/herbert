#!/usr/bin/env python3
# backfill_ref.py -- reference builder + grader for "backfill" (native-codegen link 59 / kernel-arc link 43); this link's CONTRACT is bootstrap/tests/run_native_codegen_link59.sh (+ its _mutation.sh). HEADER ANCESTRY (blind-audit A3, 2026-08-29): this file was cloned from larder_ref.py and the descriptive block that follows is the ANCESTOR's -- kept as provenance, NOT as this file's spec. Ancestor header:
...
def build_code(kstack, kend, mut=None, stage='full', npages=1, fsdel=False, fsreuse=False):
...
def build_elf(mut=None, stage='full', npages=1, fsdel=False, fsreuse=False):   # cleave L847-859 + growheap npages + delete fsdel + backfill fsreuse (fsreuse=False == delete byte-for-byte)
    code0,_=build_code(0,0,mut,stage,npages,fsdel,fsreuse); clen=len(code0)
    memsz=12+clen+16384
    kstack=LOAD+memsz; kend=LOAD+memsz
    code,labels=build_code(kstack,kend,mut,stage,npages,fsdel,fsreuse); assert len(code)==clen,(len(code),clen)
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'backfill', cd['nprocs'])
