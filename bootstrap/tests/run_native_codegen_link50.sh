...
if have_bochs; then
    ...
    for attempt in 1 2 3; do
        ...
        else fail_test "(C) Bochs (feeder SENT + ran through shutdown; guest RECEIPT unproven feeder-side -- a lone RED may be a capture-class flake, re-derive per the parley replay discriminator) -> $(python3 "$REF" gradetess "$work/b" "$KEND" "$K" "$SEED" 2>&1 | tr '\n' ';')"; fi
        bochs_done=1; break
    done
