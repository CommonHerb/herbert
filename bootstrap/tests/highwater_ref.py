...
# kernel (additivity). The future compiler emit-mode marker is `-- emit: multiboot32-larder`.
#
# WHAT larder ADDS over cairn: three syscalls -- SYS_ALLOC (eax=9; EBX=size) -> ptr in eax (0=OOM); SYS_FREE (eax=10;
# ECX=ptr) -> 0; SYS_DUMP (eax=11) -> the kernel emits the live readback. The allocator is FIRST-FIT over a DEDICATED
# heap POOL page, split-on-alloc, address-ordered coalesce-on-free.
#
# THE DESIGN (the production hardenings over the STEP-0 prototype):
#   * DEDICATED POOL PAGE carved the lodger way: the FROZEN region bump-scan (which avoids excl[] using the MB memory
#     map) mints ONE EXTRA page beyond the K proc region pages; pool_base = that page's base. The DATA spans live in
#     this page, mapped USER (CPL3 r/w) so the module can write a sentinel THROUGH a returned ptr; the free-list
#     METADATA lives in SUPERVISOR cells (the chunk[] descriptor array), so do_free NEVER dereferences a module ptr.
#     (STEP-0 reused proc0's User region page -- sharing it with the module's stack; production gives the heap its OWN
#     page, distinct from proc0's stack page.)
#   * Free-list = a CONTIGUOUS, GAP-FREE, ADDRESS-SORTED {base,len,used} struct array (invariant base[i+1]==base[i]+
#     len[i]) so address-adjacency == array-adjacency. first-fit search; split = a BACKWARD rep movsd insert; coalesce
#     = a FORWARD rep movsd delete; BOTH prev- and next-neighbour coalesce are forced.
#   * KERNEL-EMITTED WITNESS: the kernel emits each SYS_ALLOC returned ptr (0xE0 + 4 LE bytes) and, on SYS_DUMP, walks
#     the live chunks in ADDRESS order emitting (4 ptr bytes + the 4-byte sentinel it reads back through that ptr)
#     framed by 0xE1..0xE2. The module relays NOTHING. SYS_ALLOC reads ONLY EBX; SYS_FREE ONLY ECX. The do_read/sched
#     debugcon noise (0xC0/0xC1, 0xCC/0xCD) is STRIPPED off the larder trace path (do_read spin-polls; PIT unarmed).
#   * INPUT VALIDATION: SYS_ALLOC rejects size==0 / size>pool_size (a degenerate / overflow size would build a
#     zero-length chunk the module then overruns -- the durable stosd / cairn carry-check class). do_free matches a
#     chunk by EXACT base (no module deref), so a double-free / interior-ptr / unowned / out-of-pool free is a clean
#     no-op (robustness legs in larder_latebound.py confirm the allocator survives each).
#
# THE BINDING LAYERS:
#   (1) RUNTIME OUTPUT (PRIMARY): a LATE-BOUND, author-unknown alloc/free sequence (seed chosen AFTER freeze; sizes +
#       sentinels seed-derived; cumulative >> pool; non-LIFO) forces first-fit + split + prev-coalesce + next-coalesce
#       + non-MRU reuse -- a tight pool means the big allocs fit ONLY via coalesce. Forge mutants (bump / freenoop /
#       nosplit / nocoalesce / noprevmerge / nonextmerge) DIVERGE from the kernel-emitted golden -> RED. seed-diff gx/gy
#       RED.
#   (2) CONFUSED-DEPUTY / INPUT-VALIDATION: nosizewrap (drop the size reject) accepts alloc(0) -> emits a nonzero ptr
#       to a 0-length chunk -> RED; nointeriorfree (match a chunk by range not exact base) frees the containing chunk on
#       an interior-ptr free -> the live readback loses a chunk -> RED.
#   (3) WHITE-BOX (assert_larder): pins the first-fit search loop + the split-remainder insert + the prev/next coalesce
#       deletes EXACTLY-ONCE and REACHABLE (branch TARGETs, not mere presence), the ABI reads-only-ebx/ecx, the
#       kernel-emit path, and the size-wrap reject. Mirrors assert_cairn/assert_durability.
#
# Built additively on the FROZEN cairn lineage (cairn_ref started as a structural copy of cleave_ref): lodger parse +
# nokta paging-U/S + geeking fault->continue + tickover preempt + rollcall table + tenement reclaim + homestead grow +
# furlough block/wake + tessera alias + cleave COW + lethe remap + platter disk-read + durable disk-write + cairn FS.
# All of that machinery is inherited; under larder's K=1 + IF=0 single-prober scenario the preempt/multi-proc/block
# machinery is INERT (kept so the cairn lineage is byte-structural and assert_cairn/lethe/platter/durability all pass).
#
...
# ===== larder (kernel-arc link 40 / native-codegen link 56) STEP-0 NEW: a general-purpose DYNAMIC HEAP ALLOCATOR. =====
# SYS_ALLOC (eax=9; EBX=size) -> ptr in eax (0 = OOM); SYS_FREE (eax=10; ECX=ptr) -> 0; SYS_DUMP (eax=11) -> kernel-emits
# the live readback. FIRST-FIT, split-on-alloc, address-ordered coalesce-on-free. The free-list METADATA lives in kernel
# (Supervisor) cells; the DATA spans live in proc0's User region (CPL3 r/w). THE WITNESS IS KERNEL-EMITTED: the kernel
# emits the returned pointer for EACH SYS_ALLOC (marker 0xE0 + 4 LE ptr bytes) and, on SYS_DUMP, walks the live chunks in
# ADDRESS order emitting (4 ptr bytes + 4 sentinel bytes the module wrote through that ptr) framed by 0xE1..0xE2. The
# module relays NOTHING -- the SYS_ALLOC arm reads ONLY EBX (size), SYS_FREE ONLY ECX (ptr); no hidden hint channel.
SYS_ALLOC=9; SYS_FREE=10; SYS_DUMP=11
# ===== delete (kernel-arc link 42 / native-codegen link 58) NEW: mutable-FS DELETE (tombstone a named record). =====
# SYS_FS_DEL (eax=12; EBX=name_ptr, 16 bytes) -> tombstones the matching dir slot's valid:=0 IN PLACE + flush; returns
# eax=1 (deleted) / 0 (not found). ADDITIVE (TYPE-II) on the FROZEN growheap kernel: the `fsdel` build flag defaults to
# False (the DEL dispatch entry + arm are OMITTED -> build_elf(fsdel=False) reproduces growheap BYTE-FOR-BYTE); fsdel=True
# adds the arm. On the frozen growheap kernel eax=12 is an UNKNOWN opcode -> the dispatch falls through to SYS_EXIT -> NO
# delete -> the record survives -> the 3-boot reboot differential is RED. The `valid:u32` tombstone field is ALREADY
# honored by the frozen GET (skips valid!=1) + PUT (counts valid==1), so ZERO on-disk format change. The future compiler
# emit-mode marker is `-- emit: multiboot32-delete`.
SYS_FS_DEL=12
SYS_FALLOC=13              # link45 NEW (highwater): allocate a fresh physical frame TOP-DOWN from the author-unknown region_hi, map it non-identity at the reused lethe alias V (+invlpg), kernel-EMIT its physical address
SYS_HWDUMP=14             # link45 NEW (highwater): for each allocated frame, kernel re-maps V<-frame (+invlpg), reads back, EMITS addr+payload -- the distinctness witness (single-frame/no-invlpg forge -> all readbacks equal -> RED)
# link45 (highwater) constants -- a runtime free-frame allocator off an AUTHOR-UNKNOWN memory map. Reuses the lethe alias
# window V_VADDR (already excl-reserved + page-table-installed) as the single remap window; reuses larder al_* scratch +
# the chunk[] block (both inert in highwater mode) so highwater=False stays tract-byte-identical (NO new cell).
HW_N        = 6           # frames the witness prober allocates top-down (bounded; honest scope -- N fresh frames, not a production allocator)
HW_MAXFRAMES = 32         # HARD per-boot SYS_FALLOC cap (< the 48-dword hw_frames[] capacity; > HW_N). Bounds BOTH the
                          # hw_frames[] index (no CPL0 overrun past the array) AND the cursor descent (<=32*4KiB=128KiB below
                          # region_hi, so a hostile CPL3 loop can NEVER descend into the kernel/page-table/proc region and
                          # alias it User|RW). Closes the two Codex-found holes in the unbounded syscall surface.
HW_TAG      = 0xABCD0000  # payload base; P_i = HW_TAG | ((seed&0xFF)<<8) | i  (seed-derived + i-distinct => late-bound, single-frame forge caught)
HW_AMARK    = 0xF0        # per-SYS_FALLOC: emit 0xF0 ++ le32(frame_phys)  (kernel-emitted provenance; author-unknown via -m)
HW_DBEGIN   = 0xF1        # SYS_HWDUMP begin
HW_DEND     = 0xF3        # SYS_HWDUMP end  (per-entry: le32(frame_phys) ++ le32(readback))
def hw_payload(seed,i): return (HW_TAG | ((seed & 0xFF) << 8) | (i & 0xFF)) & 0xFFFFFFFF
LARDER_POOL=168                 # the heap pool size in bytes (carved from proc0's User region; sub-page, tight: peak live==pool)
LARDER_MAXCHUNKS=16             # free-list capacity (chunk descriptors); peak on the witness is 4, 16 is ample
LARDER_MAGIC=bytes([0x4C,0x41,0x52,0x44,0x45,0x52,0xA5,0x5A])  # "LARDER\xA5\x5A": the kernel emits this once before iret-to-proc0 so the host grader can locate the larder trace unambiguously after all boot-time debugcon noise
# The future compiler emit-mode marker for this link (baked into gen-1): `-- emit: multiboot32-larder`.
...
FS_OFF_LBA   = 24           # u32 data_lba (the sector holding the payload / the START of a multi-sector RUN under varsize)
# ===== tract (link 44): VARIABLE-SIZE / multi-sector durable files (varsize=True) =====
# A record payload may exceed one sector: it occupies a CONTIGUOUS RUN of nsectors=ceil(len/512) data sectors starting
# at data_lba. The dir entry format is UNCHANGED (len -> nsectors, data_lba -> run start). The data window is EXTENDED.
# Allocation = recompute-the-free-map-from-the-DIRECTORY (mark each live entry's run used in fsused[]; first-fit the
# lowest free run of `need`; coalesce is implicit -- gaps are maximal). varsize=False keeps the frozen single-sector path
# (FS_MAXLEN=512, FS_DATA_HI window) BYTE-FOR-BYTE; the varsize arm uses TRACT_MAXLEN / TRACT_DATA_HI immediates instead.
TRACT_RUN_MAX = 4                          # max sectors per record
TRACT_MAXLEN  = TRACT_RUN_MAX * 512        # 2048: max payload under varsize (frozen FS_MAXLEN stays 512)
TRACT_DATA_HI = FS_DATA_LO + 32            # extended data-sector window HI (exclusive): 32 sectors for varsize
TRACT_W       = TRACT_DATA_HI - FS_DATA_LO # number of data sectors in the varsize window (== free-map size)
assert TRACT_DATA_HI > FS_DATA_HI, 'the varsize window must EXTEND past the frozen cairn FS window (additive)'
assert TRACT_W <= 256, 'window sector indices must fit a byte (the fsused[] free-map is byte-indexed)'
assert TRACT_RUN_MAX <= TRACT_W, 'a single record run must fit the window'
...
# ===== growheap (kernel-arc link 41 / native-codegen link 57) NEW: GROW THE HEAP -- a MULTI-PAGE heap pool with
# cross-page split + address-ordered CROSS-PAGE-BOUNDARY coalesce. ADDITIVE (TYPE-II) on the FROZEN larder lineage:
# build_code/build_elf(npages=1) reproduce the larder bytes EXACTLY (the byte-identical default); npages=GROWHEAP_NPAGES
# is the NEW `multiboot32-growheap` emit mode -- a contiguous NPAGES-page User pool. larder explicitly DEFERRED multi-
# page growth ("no multi-page heap growth"); growheap pays it. The genuine first-fit/split/coalesce arms are REUSED
# BYTE-IDENTICALLY from larder (they already span a contiguous pool, so cross-page coalesce emerges once pool_size>PGSZ).
GROWHEAP_PGSZ   = 0x1000        # page size (the cross-page boundary the witness straddles)
GROWHEAP_NPAGES = 2             # the multi-page pool: NPAGES contiguous User pages (NPAGES>=2 forces multi-page)
GROWHEAP_CAP    = GROWHEAP_NPAGES * GROWHEAP_PGSZ   # 8192: peak live-set == CAP forces the pool to span > 1 page
# The pool is a FIXED, identity-mapped, NPAGES-page CONTIGUOUS region (the lethe-frame reservation pattern), reserved
# from the region bump allocator via a fixed excl[] entry and flipped User. A fixed CONTIGUOUS region is chosen over the
# naive "mint +NPAGES bump pages" because consecutive bump pages are separated by a GROWBYTES grow window (so they are
# NON-contiguous) -- a contiguous multi-page span is REQUIRED for cross-page coalesce. pool_base = GROWHEAP_POOL_VADDR
# (the grader reads pool_base from the kernel cell dump, so the fixed base is transparent to it). The larder +1 bump
# pool page is STILL minted (slot nprocs, unused for npages>1) so the MAXPROC+1 region-array boundary invariant below is
# UNCHANGED (the bump loop's max written region-array index stays nprocs; the fixed pool region uses NO bump slot).
GROWHEAP_POOL_VADDR = 0x700000  # 7 MiB: page-aligned, identity-mapped (<16 MiB), in the free gap between lethe A(0x600000) and V(0x800000)
GROWHEAP_MAGIC = LARDER_MAGIC   # reuse larder's trace-locator banner bytes (byte-identical; growheap is a separate boot, no ambiguity)
# The future compiler emit-mode marker for this link (baked into gen-1): `-- emit: multiboot32-growheap`.
assert GROWHEAP_NPAGES >= 2, 'growheap forces a MULTI-page pool (NPAGES>=2)'
assert GROWHEAP_POOL_VADDR % GROWHEAP_PGSZ == 0, 'the pool base must be page-aligned (so internal page boundaries land at pool_base + k*PGSZ)'
assert GROWHEAP_POOL_VADDR + GROWHEAP_CAP <= NPT*0x400000, 'the whole pool must lie inside the 16 MiB identity map'
# the pool region must be DISJOINT from every lethe alias page / frame window (all reserved, all identity-mapped).
_GH_LO, _GH_HI = GROWHEAP_POOL_VADDR, GROWHEAP_POOL_VADDR + GROWHEAP_CAP
for _lo,_hi in [(A_VADDR,A_VADDR+0x1000),(V_VADDR,V_VADDR+0x1000),(B_VADDR,B_VADDR+0x1000),
                (min(F_FRAME,FP_FRAME),max(F_FRAME,FP_FRAME)+0x1000)]:
    assert _GH_HI <= _lo or _hi <= _GH_LO, 'growheap pool region [%x,%x) overlaps a reserved lethe window [%x,%x)'%(_GH_LO,_GH_HI,_lo,_hi)
assert GROWHEAP_POOL_VADDR >= 0x100000, 'the pool must sit above the kernel image base'
# larder excl[] boundary need: 9 fixed (kernel/mb/cm/elf/mmap + lethe A/V/B + frame-pair window) + 2 per module
# (modstart/modend window + stack window, up to MAXPROC modules) + 1 per minted region page. larder mints nprocs+1 region
# pages (the K proc pages PLUS the dedicated heap-pool page), so at the K=MAXPROC boundary that is MAXPROC+1 region excl
# entries -> 9 + 2*MAXPROC + (MAXPROC+1) = 34. The old NEXCL=9+3*MAXPROC=33 was ONE SHORT (it omitted the +1 pool page);
# the old assert (>=8+3*MAXPROC=32) was too loose to catch it. (Codex/link-verify caught this MAXPROC-boundary overflow.)
NEXCL=9+2*MAXPROC+(MAXPROC+1)   # = 34
assert NEXCL >= 9 + 2*MAXPROC + (MAXPROC+1), 'excl[] must hold 9 fixed + 2/module + 1/region for K up to MAXPROC, where larder mints nprocs+1 region pages (the +1 = the dedicated heap pool page)'
# growheap (npages>1) excl[] boundary: it adds a 10th FIXED entry (the fixed 0x700000 pool region) but, in exchange,
# does NOT mint larder's +1 bump pool page (the inc is gated `if not GH` in the carve). So GH max = 10 fixed + 2/module
# + 1/region * MAXPROC = 10 + 3*MAXPROC = 34 == NEXCL. This static guard makes the GH boundary bite-proof (an 11th fixed
# entry would fail import). EMPIRICALLY VERIFIED by a K=MAXPROC boot: nexcl == 34, not 35.
assert NEXCL >= 10 + 3*MAXPROC, 'growheap excl[]: 10 fixed (larder 9 + the fixed pool region) + 3 per proc (2/module + 1 region page, no +1 pool page in GH mode) must fit for K up to MAXPROC'
...
SCALARS=['mbinfo','flags','cmdline','elflo','elfhi','mm_lo','mm_hi','region_lo','region_hi',
         ...
         'pool_base','pool_size','nchunks','al_size','al_ptr','al_i','al_k','al_ret','al_cur']  # link40 (larder): heap pool base/size, free-list length, + alloc/free/dump scratch (al_cur = the M-bump cursor; unused/0 in genuine)
...
# larder MAXPROC-boundary fix: the region bump-loop + the heap-pool carve index alloc_lo/alloc_hi/grow_floor up to
# index nprocs (pool_base = alloc_lo[nprocs]); since larder mints nprocs+1 region pages, the max written index is nprocs,
# which at the K=MAXPROC boundary is MAXPROC. So those THREE arrays need MAXPROC+1 entries (indices 0..MAXPROC); a plain
# MAXPROC sizing made alloc_lo[MAXPROC] alias alloc_hi[0] and corrupt boot state. The other PARR arrays are only indexed
# up to nprocs-1 (real procs), so they stay MAXPROC. (Codex/link-verify caught this; the K=1 gates never hit it.)
_POOL_INDEXED = {'alloc_lo','alloc_hi','grow_floor'}      # indexed at index nprocs (== the heap-pool slot) -> need MAXPROC+1
def _parr_n(nm): return (MAXPROC+1) if nm in _POOL_INDEXED else MAXPROC
CELLS=[]
CELLS+= SCALARS
_ARRBASE={}
for nm in PARR:
    _ARRBASE[nm]=len(CELLS)
    CELLS+=[f'{nm}#{i}' for i in range(_parr_n(nm))]
_EXCLLO=len(CELLS); CELLS+=[f'excl_lo#{i}' for i in range(NEXCL)]
_EXCLHI=len(CELLS); CELLS+=[f'excl_hi#{i}' for i in range(NEXCL)]
# link40 (larder): the free-list chunk-descriptor array -- a CONTIGUOUS block of LARDER_MAXCHUNKS structs, each
# {base:u32, len:u32, used:u32} = 12 bytes. Kept ALWAYS address-sorted AND gap-free (it partitions [pool_base,pool_base+
# pool_size) into adjacent chunks), so address-adjacency == array-adjacency -> coalesce just merges array neighbours.
_CHUNKBASE=len(CELLS); CELLS+=[f'chunk#{i}' for i in range(LARDER_MAXCHUNKS*3)]
...
def chunk_base(): return CELLBASE+_CHUNKBASE*4   # link40: absolute addr of chunk#0 (struct[0].base); struct i field f at chunk_base()+i*12+f
...
# ===== larder MAXPROC-boundary INVARIANT (Codex/link-verify catch; a gate-level guard against a regression) =====
# The heap-pool region descriptor lives at alloc_lo[nprocs] (one PAST the K proc pages); at the K=MAXPROC boundary that is
# index MAXPROC. The three region arrays the bump-loop + pool carve index up to nprocs (alloc_lo/alloc_hi/grow_floor) MUST
# therefore be MAXPROC+1 entries so alloc_lo[MAXPROC] is a REAL cell that does NOT alias the next array (a MAXPROC sizing
# made alloc_lo[MAXPROC]==alloc_hi[0] and corrupted boot state at nprocs==MAXPROC). And NEXCL must cover the boundary need.
assert arr('alloc_lo')+MAXPROC*4 < arr('alloc_hi'),  'alloc_lo[MAXPROC] (the K=MAXPROC heap-pool slot) aliases alloc_hi -- region arrays must be sized MAXPROC+1'
assert arr('alloc_hi')+MAXPROC*4 < arr('grow_floor'),'alloc_hi[MAXPROC] aliases grow_floor -- region arrays must be sized MAXPROC+1'
assert arr('grow_floor')+MAXPROC*4 < arr('started'), 'grow_floor[MAXPROC] aliases started -- region arrays must be sized MAXPROC+1'
assert NEXCL >= 9 + 2*MAXPROC + (MAXPROC+1),          'NEXCL below the K=MAXPROC boundary need (9 fixed + 2/module + (MAXPROC+1)/region pages incl the heap pool)'


def build_code(kstack, kend, mut=None, stage='full', npages=1, fsdel=False, fsreuse=False, varsize=False, highwater=False):
    # tract (link 44): varsize=False reproduces the backfill kernel BYTE-FOR-BYTE (every `if varsize:` block is skipped,
    # no new cells, the fsused[] buffer is NOT emitted). varsize=True switches SYS_FS_PUT/GET to MULTI-SECTOR contiguous
    # runs with a recompute-from-directory first-fit allocator (the variable-size durable file). Reuses al_*/fs_* scratch
    # cells (unused during an FS arm) so NO cell is added (the cell-storage blob + the runtime cell-dump stay frozen).
    # growheap: npages=1 reproduces the larder kernel BYTE-FOR-BYTE (every `if npages>1:` block is skipped and every
    # parametrized immediate collapses to its larder value); npages=GROWHEAP_NPAGES builds the multi-page growheap pool.
    # backfill (link 43): fsreuse=False reproduces the DELETE kernel BYTE-FOR-BYTE (the do_fs_put allocator stays
    # append-by-count); fsreuse=True switches SYS_FS_PUT to FIRST-FREE-SLOT allocation (pays LEDGER D22 -- reuse a
    # tombstoned hole instead of colliding with a live entry after a non-tail delete). The parametrize-frozen-ref
    # discipline: the fsreuse=False default must reproduce delete byte-for-byte (proven by cmp in the gate).
    a=Asm()
    a._ctr=0
    assert npages>=1, 'npages>=1'
    GH = (npages>1)                                   # the growheap (multi-page) mode flag
    ...
    def ata_write_sector(buf_label, lba_cell, flush=True):
        ...
        # flush defaults True (every existing caller -- PUT's two writes -- stays BYTE-IDENTICAL); the DELETE arm passes
        # flush=(mut!='noflush') so M-noflush emits the WRITE SECTORS without the CACHE FLUSH -> the tombstone never
        # reaches the medium -> it is LOST across the BOOT-2->BOOT-3 reboot -> the deleted record reappears -> RED.
    ...
    # ===== link40 (larder) free-list helpers (DS/ES=KDATA flat assumed; chunk array is gap-free + address-sorted) =====
    CHB = chunk_base()
    def ch_esi_from_ecx():                                 # esi <- &chunk[ecx]  (esi = CHB + ecx*12)
        a.raw(0x6B,0xF1,0x0C)                              # imul esi,ecx,12
        a.raw(0x81,0xC6); a.blob(le32(CHB))               # add esi, CHB
    def ch_edi_from_ecx():                                 # edi <- &chunk[ecx]
        a.raw(0x6B,0xF9,0x0C)                              # imul edi,ecx,12
        a.raw(0x81,0xC7); a.blob(le32(CHB))               # add edi, CHB
    def emit_larder_split():
        # split chunk[al_i] into a USED head of [al_size] bytes + a FREE remainder, inserted at index al_i+1.
        # (1) shift entries [i+1, nchunks) UP by one slot (3 dwords) via a BACKWARD rep movsd (overlap-safe).
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))      # mov ecx,[al_i]
        mem(cell('nchunks')); a.raw(0x29,0xC8); a.raw(0x48)   # eax = nchunks - i - 1  (entries to shift)
        a.raw(0x8D,0x0C,0x40)                             # lea ecx,[eax+eax*2]  (ecx = 3*entries = dword count)
        mem(cell('nchunks')); a.raw(0x6B,0xF0,0x0C)       # eax=nchunks ; imul esi,eax,12
        a.raw(0x81,0xC6); a.blob(le32((CHB-4)&0xFFFFFFFF))   # add esi, CHB-4   (esi = src_end = last dword of [0,nchunks))
        a.raw(0x89,0xF7); a.raw(0x83,0xC7,0x0C)           # mov edi,esi ; add edi,12  (edi = dest_end)
        a.raw(0xFD); a.raw(0xF3,0xA5); a.raw(0xFC)        # std ; rep movsd ; cld
        # (2) write the new FREE remainder at index i+1, and shrink chunk[i] to [al_size].
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))      # mov ecx,[al_i]
        ch_edi_from_ecx()                                 # edi -> chunk[i]  (untouched by the shift)
        # [i+1].base = base_i + size. M-badsplit corrupts the remainder BASE (off by +4) -> the gap-free invariant breaks
        # and the next alloc that lands on the remainder returns the WRONG offset -> the emitted trace DIVERGES -> RED.
        a.raw(0x8B,0x07); a.raw(0x03,0x05); a.blob(le32(cell('al_size')))   # eax = base_i + size
        if mut=='badsplit':
            a.raw(0x83,0xC0,0x04)                         # add eax,4  (WRONG remainder base)
        a.raw(0x89,0x47,0x0C)                             # [i+1].base = eax
        # [i+1].len = len_i - size. M-badsplitlen corrupts the remainder LEN (off by -4) -> a later exact-fit alloc OOMs
        # where genuine fits (the remainder is undersized) -> the emitted trace DIVERGES -> RED.
        a.raw(0x8B,0x47,0x04); a.raw(0x2B,0x05); a.blob(le32(cell('al_size')))   # eax = len_i - size
        if mut=='badsplitlen':
            a.raw(0x83,0xE8,0x04)                         # sub eax,4  (WRONG remainder len)
        a.raw(0x89,0x47,0x10)                             # [i+1].len = eax
        a.raw(0xC7,0x47,0x14,0x00,0x00,0x00,0x00)         # [i+1].used = 0
        a.raw(0x8B,0x05); a.blob(le32(cell('al_size'))); a.raw(0x89,0x47,0x04)  # [i].len = size
        a.raw(0xFF,0x05); a.blob(le32(cell('nchunks')))   # nchunks++
    def emit_larder_delete():
        # delete chunk[al_k]: shift entries (al_k, nchunks) DOWN by one slot via a FORWARD rep movsd; nchunks--.
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_k')))      # mov ecx,[al_k]  (k)
        a.raw(0x6B,0xF1,0x0C); a.raw(0x81,0xC6); a.blob(le32((CHB+12)&0xFFFFFFFF))  # esi = &chunk[k+1] (src)
        # edi = &chunk[k] (dest). M-baddelete off-by-ONE-DWORD the DEST (sub edi,8 -> &chunk[k]+4) so the forward shift
        # writes every struct field one dword high -> the gap-free address-sorted array is left MALFORMED -> a later
        # alloc/free reads a corrupted chunk -> the emitted trace DIVERGES -> RED. (Bounded: the dword count is unchanged,
        # so the shift stays within the 16-slot chunk array -- no runaway rep movsd.)
        a.raw(0x89,0xF7)                                  # mov edi,esi
        if mut=='baddelete':
            a.raw(0x83,0xEF,0x08)                         # sub edi,8   (dest = &chunk[k] + 4 -- off-by-one-dword)
        else:
            a.raw(0x83,0xEF,0x0C)                         # sub edi,12  (dest = &chunk[k])
        mem(cell('nchunks')); a.raw(0x29,0xC8); a.raw(0x48)  # eax = nchunks - k - 1
        a.raw(0x8D,0x0C,0x40)                             # ecx = 3*eax (dword count)
        a.raw(0xFC); a.raw(0xF3,0xA5)                     # cld ; rep movsd
        a.raw(0xFF,0x0D); a.blob(le32(cell('nchunks')))   # nchunks--
    ...
    if GH:
        # growheap: reserve the FIXED, contiguous NPAGES-page heap-pool region [POOL_VADDR, POOL_VADDR+CAP) from the
        # region bump allocator so no proc page lands on it (one excl entry; the lethe-frame reservation pattern). For
        # npages=1 this block is skipped -> byte-identical to larder. NEXCL=34 has ample room for this extra fixed entry.
        excl_push(lambda:(a.raw(0xB8),a.blob(le32(GROWHEAP_POOL_VADDR))),
                  lambda:(a.raw(0xB8),a.blob(le32(GROWHEAP_POOL_VADDR+npages*GROWHEAP_PGSZ))))                   # growheap pool region
    a.raw(0x31,0xC9)
    a.lbl('mexcl')
    ...
    # ===== bump allocator (cleave L302-345; mints K region pages + ONE EXTRA = larder's DEDICATED POOL page) =====
    # larder PRODUCTION HARDENING: carve a DEDICATED heap pool page the lodger way -- mint tmp0 = min(MSLOTS,nprocs)+1
    # pages so the FROZEN bump-scan (which avoids excl[] via the MB memory map and reserves each minted page in excl[])
    # hands out ONE MORE genuinely-free page beyond the K proc region pages. That extra page (slot nprocs) becomes the
    # heap pool; proc0's stack keeps its OWN region page (slot 0). The +1 is K=1-scoped (nprocs+1 <= MAXPROC holds for
    # the single larder prober). The grow-window clear below iterates `nprocs` (real procs only), so the pool slot's
    # grow window is left present+Supervisor (unused, harmless) -- it never touches proc0's grow clear.
    a.raw(0xB8); a.blob(le32(MSLOTS))
    a.raw(0x3B,0x05); a.blob(le32(cell('nprocs'))); a.j(JBE,'mbok')
    a.raw(0xA1); a.blob(le32(cell('nprocs')))
    a.lbl('mbok')
    if not GH: a.raw(0x40)                             # larder: inc eax -> tmp0 = min(MSLOTS,nprocs)+1 (the +1 dedicated pool page). growheap: the pool is the FIXED 0x700000 region (excl-reserved at line ~527), so the +1 bump page is REDUNDANT -- minting it AND the fixed entry pushes nexcl to 11+3*nprocs = 35 > NEXCL=34 at nprocs==MAXPROC (a LATENT one-past-end excl[] overflow the K=1 witness never hits; Codex + an empirical K=8 boot caught it -- the larder MAXPROC class). Dropping it in GH mode -> 10+3*MAXPROC = 34 == NEXCL. npages=1 KEEPS the 0x40 -> byte-identical to larder. (pool_base is overridden to the fixed region below, so the now-unwritten alloc_lo[nprocs] read is dead.)
    ...
    # ===== link40 (larder): carve the heap pool from the DEDICATED page (slot nprocs) + init the free-list =====
    # PRODUCTION: the pool lives at the BASE of its OWN bump-scanned page (alloc_lo[nprocs]) -- a page DISTINCT from
    # proc0's stack page (alloc_lo[0]). The pool is kept tight (LARDER_POOL bytes at the page base) so the big late-bound
    # allocs fit ONLY via coalesce; the rest of the page is unused. The DATA spans are this page (mapped User below for
    # CPL3 sentinel-writes, identity-mapped so the CPL0 readback can read them); the free-list METADATA stays in the
    # Supervisor chunk[] cells, so do_free never dereferences a module ptr. The host grader reads pool_base from the
    # cell dump, so the dedicated-page move is transparent to it.
    # growheap: pool_size is LARDER_POOL for npages=1 (byte-identical) and npages*PGSZ for the multi-page pool. The
    # M-singlepage mutant CAPS pool_size (and chunk[0].len) to ONE page -> the witness's page-2 alloc (C) OOMs where the
    # genuine 2-page pool succeeds (the kernel-emitted trace DIVERGES -> RED). The genuine multi-page build is byte-
    # identical to larder's carve EXCEPT these two immediates + the fixed pool_base (no new cells, no arm changes).
    if npages == 1:
        POOL_SIZE_VAL = LARDER_POOL
    else:
        POOL_SIZE_VAL = GROWHEAP_PGSZ if mut=='singlepage' else (npages*GROWHEAP_PGSZ)
    a.raw(0x8B,0x0D); a.blob(le32(cell('nprocs')))                   # mov ecx,[nprocs]   (the dedicated pool slot index)
    ld('eax',arr('alloc_lo'),'ecx')                                  # eax = alloc_lo[nprocs] (the larder +1 bump page)
    if GH:
        a.raw(0xB8); a.blob(le32(GROWHEAP_POOL_VADDR))               # growheap: pool_base = the FIXED contiguous NPAGES-page region (override the unused bump page)
    mme(cell('pool_base'))                                           # pool_base = (alloc_lo[nprocs] | growheap fixed region)
    mmi(cell('pool_size'), POOL_SIZE_VAL)                            # pool_size = N (larder) | npages*PGSZ (growheap) | PGSZ (M-singlepage)
    mmi(cell('nchunks'), 1)                                          # one chunk to start
    mem(cell('pool_base')); a.raw(0xA3); a.blob(le32(chunk_base()))  # chunk[0].base = pool_base
    a.raw(0xC7,0x05); a.blob(le32(chunk_base()+4)); a.blob(le32(POOL_SIZE_VAL))   # chunk[0].len = pool_size
    a.raw(0xC7,0x05); a.blob(le32(chunk_base()+8)); a.blob(le32(0))             # chunk[0].used = 0 (free)
    ...
    # larder: flip the DEDICATED pool page (slot nprocs) -> User, so the module's CPL3 stores THROUGH returned ptrs land
    # (a Supervisor page would #PF the sentinel write). The CPL0 do_dump readback reads it fine (page perms gate on CPL).
    a.raw(0x8B,0x0D); a.blob(le32(cell('nprocs')))     # mov ecx,[nprocs]  (the pool slot)
    flip_pg(arr('alloc_lo'),'ecx',True)                # pool page -> User
    if GH:
        # growheap: flip the FIXED, contiguous NPAGES-page pool region -> User. Each page is identity-mapped at boot
        # (PTE = page|3); we overwrite it with page|7 (present+RW+User) so the module's CPL3 sentinel-writes THROUGH the
        # returned ptrs land, while the CPL0 do_dump readback still reads them (page perms gate on CPL). DS=KDATA here.
        # Mirrors lethe's PTE installs; the subsequent cr3 reload flushes. For npages=1 this block is skipped.
        for _i in range(npages):
            _pg = GROWHEAP_POOL_VADDR + _i*GROWHEAP_PGSZ
            a.raw(0xC7,0x05); a.absR('pt',(_pg>>12)*4); a.blob(le32(_pg|7))   # PTE[pool page i] <- page | P|RW|User (identity+User)
    ...
    a.lbl('gw_p')
    a.raw(0x3B,0x3D); a.blob(le32(cell('nprocs'))); a.j(JAE,'gw_pdone')   # larder: iterate REAL procs (nprocs), NOT tmp0 (the +1 pool slot has no grow window to clear)
    ...
    # ===== link40 (larder): emit the MAGIC banner so the host grader can locate the larder trace past all boot noise =====
    for _mb in LARDER_MAGIC: outi(_mb)
    ...
    a.raw(0x83,0xF8,SYS_ALLOC); a.j(JE,'do_alloc')         # eax==9  -> ALLOC (link40 NEW -- larder heap)
    a.raw(0x83,0xF8,SYS_FREE);  a.j(JE,'do_free')          # eax==10 -> FREE  (link40 NEW)
    a.raw(0x83,0xF8,SYS_DUMP);  a.j(JE,'do_dump')          # eax==11 -> DUMP  (link40 NEW -- kernel-emitted live readback)
    if fsdel:
        a.raw(0x83,0xF8,SYS_FS_DEL); a.j(JE,'do_fs_del')   # eax==12 -> FS DEL (link42 NEW -- mutable-FS DELETE; gated by fsdel so fsdel=False == growheap byte-for-byte. On the FROZEN growheap kernel eax==12 is unknown -> falls through to SYS_EXIT below -> no delete -> RED.)
    if highwater:
        a.raw(0x83,0xF8,SYS_FALLOC); a.j(JE,'do_falloc')   # eax==13 -> FALLOC (link45 NEW -- highwater; gated by highwater so highwater=False == tract byte-for-byte. On the FROZEN tract kernel eax==13 is unknown -> falls through to SYS_EXIT below -> the prober gets no frame -> RED.)
        a.raw(0x83,0xF8,SYS_HWDUMP); a.j(JE,'do_hwdump')   # eax==14 -> HWDUMP (link45 NEW -- per-frame re-map + invlpg + readback + kernel-emit; the distinctness witness)
    ...
    # link40 STEP-0: with K=1 + IF=0 the furlough BLOCK arm just bounces through sched_switch (no peer to run) and emits
    # the 0xCC..0xCD wake dump -- noise that interleaves with the larder trace. Replace it with a simple SPIN-POLL: if the
    # byte isn't ready yet, re-poll LSR. (The feeder holds the socket; the byte arrives. Correct for a single prober.)
    a.j(None,'do_read')                               # not ready -> spin-poll LSR again
    a.lbl('do_read_ready')
    a.blob(bytes.fromhex('66baf803ec'))               # mov dx,0x3f8; in al,dx  (read RBR)
    a.raw(0x88,0xC3)                                       # mov bl,al
    # link40 STEP-0: the do_read debugcon dump (0xC0..0xC1 frame) is STRIPPED so the only OUT traffic after the magic
    # banner is the larder alloc/dump trace -- a clean, positionally-parseable stream. (The functional COM1 read stays.)
    load_udata()
    ...
    a.lbl('do_fs_put')
    if varsize:
        # ===== tract SYS_FS_PUT (varsize): MULTI-SECTOR contiguous-run PUT, recompute-from-dir first-fit =====
        load_kdata()
        a.raw(0x89,0x1D); a.blob(le32(cell('fs_nameptr')))     # [fs_nameptr]=ebx
        a.raw(0x89,0x0D); a.blob(le32(cell('fs_payptr')))      # [fs_payptr]=ecx
        a.raw(0x89,0x15); a.blob(le32(cell('fs_len')))         # [fs_len]=edx
        a.raw(0x85,0xD2); a.j(JE,'fs_put_reject')              # test edx,edx ; jz reject (len==0)
        a.raw(0x81,0xFA); a.blob(le32(TRACT_MAXLEN)); a.j(JA,'fs_put_reject')   # cmp edx,2048 ; ja reject (len>RUN_MAX*512)
        a.raw(0x8B,0x05); a.blob(le32(cell('cur')))            # eax=cur
        ld('esi',arr('alloc_lo'),'eax'); ld('edi',arr('alloc_hi'),'eax')
        a.raw(0x8B,0x1D); a.blob(le32(cell('fs_nameptr')))     # ebx=name_ptr
        a.raw(0x39,0xF3); a.j(JB,'fs_put_reject')              # cmp ebx,lo ; jb reject
        a.raw(0x89,0xDA); a.raw(0x83,0xC2,FS_NAMELEN); a.j(JB,'fs_put_reject')   # edx=ebx+16 ; jc reject (wrap)
        a.raw(0x39,0xFA); a.j(JA,'fs_put_reject')              # cmp edx,hi ; ja reject (name end>hi)
        a.raw(0x8B,0x1D); a.blob(le32(cell('fs_payptr')))      # ebx=payload_ptr
        a.raw(0x39,0xF3); a.j(JB,'fs_put_reject')              # cmp ebx,lo ; jb reject
        a.raw(0x8B,0x15); a.blob(le32(cell('fs_len')))         # edx=len
        a.raw(0x01,0xDA); a.j(JB,'fs_put_reject')              # add edx,ebx (pay end) ; jc reject (wrap)
        a.raw(0x39,0xFA); a.j(JA,'fs_put_reject')              # cmp edx,hi ; ja reject
        # need = (len+511)>>9  -> al_size  (M-trunc FORGE: need=1 single-sector -> a >512 record truncates -> readback RED;
        #   M-noceil FORGE: floor (drop +511) -> drops the partial last sector -> readback RED on a partial-tail payload)
        if mut=='trunc':
            a.raw(0xB8,0x01,0x00,0x00,0x00)                    # mov eax,1  (FORGE single-sector)
        elif mut=='noceil':
            a.raw(0x8B,0x05); a.blob(le32(cell('fs_len'))); a.raw(0xC1,0xE8,0x09)   # eax=len ; shr 9 (FLOOR)
        else:
            a.raw(0x8B,0x05); a.blob(le32(cell('fs_len')))     # eax=len
            a.raw(0x05); a.blob(le32(511)); a.raw(0xC1,0xE8,0x09)  # add eax,511 ; shr eax,9  (ceil)
        a.raw(0xA3); a.blob(le32(cell('al_size')))             # [al_size]=need
        # read the directory
        a.raw(0xC7,0x05); a.blob(le32(cell('tmp2'))); a.blob(le32(FS_DIR_LBA))
        ata_read_sector('dirbuf', cell('tmp2'))
        # ---- build free-map fsused[0..TRACT_W): zero it ----
        a.raw(0xBF); a.absR('fsused')                          # edi=fsused
        a.raw(0x31,0xC0); a.raw(0xB9); a.blob(le32(TRACT_W))   # xor eax,eax ; ecx=TRACT_W
        a.raw(0xFC,0xF3,0xAA)                                  # cld ; rep stosb
        # ---- mark loop: for j=0..FS_D-1, mark each LIVE entry's run [data_lba-LO, +ceil(len/512)) ----
        a.raw(0x31,0xC9)                                       # xor ecx,ecx (j=0)
        a.lbl('vput_mark')
        a.raw(0x83,0xF9,FS_D); a.j(JAE,'vput_markdone')        # cmp j,FS_D ; jae done
        a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))           # [al_i]=j
        a.raw(0x6B,0xF1,FS_ENTSZ); a.raw(0x81,0xC6); a.absR('dirbuf')   # esi=dirbuf+j*28
        a.raw(0x83,0x3E,0x01); a.j(JNE,'vput_marknext')        # cmp [esi+0],1 ; jne skip
        a.raw(0x8B,0x46,FS_OFF_LBA)                            # eax=[esi+24] data_lba
        a.raw(0x2D); a.blob(le32(FS_DATA_LO))                  # sub eax,FS_DATA_LO  -> ridx
        a.raw(0xA3); a.blob(le32(cell('al_ptr')))             # [al_ptr]=ridx
        a.raw(0x8B,0x46,FS_OFF_LEN)                            # eax=[esi+4] len
        a.raw(0x05); a.blob(le32(511)); a.raw(0xC1,0xE8,0x09)  # +511 ; >>9  -> rlen
        # CAP rlen at TRACT_W (a corrupt LIVE entry with a huge len must not make the mark loop run unboundedly -- Codex
        # secondary catch; a run can't exceed the window, so capping is conservative + bounds the inner loop).
        a.raw(0x3D); a.blob(le32(TRACT_W)); a.j(JBE,'vput_rlencap')   # cmp rlen,TRACT_W ; jbe ok
        a.raw(0xB8); a.blob(le32(TRACT_W))                    # mov eax,TRACT_W  (cap)
        a.lbl('vput_rlencap')
        a.raw(0xA3); a.blob(le32(cell('al_cur')))             # [al_cur]=rlen (capped)
        a.raw(0x31,0xD2)                                       # xor edx,edx (m=0)
        a.lbl('vput_markinner')
        a.raw(0x3B,0x15); a.blob(le32(cell('al_cur'))); a.j(JAE,'vput_markinnerdone')   # cmp m,rlen ; jae
        a.raw(0x8B,0x05); a.blob(le32(cell('al_ptr')))         # eax=ridx
        a.raw(0x01,0xD0)                                       # add eax,edx (ridx+m)
        a.raw(0x3D); a.blob(le32(TRACT_W)); a.j(JAE,'vput_markinnernext')   # if idx>=TRACT_W skip (defensive)
        a.raw(0xC6,0x80); a.absR('fsused'); a.raw(0x01)        # mov byte [eax+fsused],1
        a.lbl('vput_markinnernext')
        a.raw(0x42); a.j(None,'vput_markinner')                # inc m ; loop
        a.lbl('vput_markinnerdone')
        a.lbl('vput_marknext')
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))           # ecx=[al_i]=j
        a.raw(0x41); a.j(None,'vput_mark')                     # inc j ; loop
        a.lbl('vput_markdone')
        # ---- first-fit: lowest s in [0,TRACT_W-need] with fsused[s..s+need) all 0 -> runstart=FS_DATA_LO+s ----
        a.raw(0x31,0xC9)                                       # xor ecx,ecx (s=0)
        a.lbl('vput_scan')
        a.raw(0x89,0xC8); a.raw(0x03,0x05); a.blob(le32(cell('al_size')))   # eax=s ; add eax,need
        a.raw(0x3D); a.blob(le32(TRACT_W)); a.j(JA,'fs_put_reject')         # cmp s+need,TRACT_W ; ja reject (no fit)
        a.raw(0x31,0xD2)                                       # xor edx,edx (k=0)
        a.lbl('vput_scancheck')
        a.raw(0x3B,0x15); a.blob(le32(cell('al_size'))); a.j(JAE,'vput_found')   # cmp k,need ; jae -> all free
        a.raw(0x89,0xC8); a.raw(0x01,0xD0)                     # eax=s ; add eax,edx (s+k)
        a.raw(0x80,0xB8); a.absR('fsused'); a.raw(0x00)        # cmp byte [eax+fsused],0
        a.j(JNE,'vput_scannext')                               # used -> this s fails
        a.raw(0x42); a.j(None,'vput_scancheck')                # inc k ; continue
        a.lbl('vput_scannext')
        a.raw(0x41); a.j(None,'vput_scan')                     # inc s ; next candidate
        a.lbl('vput_found')
        if mut=='decoupled':
            # FORGE: ignore the first-fit scan result -> always start the run at FS_DATA_LO. In the forcing the lowest
            # sector is a LIVE survivor's, so this CLOBBERS it (raw oracle: survivor-immutability + wrong placement -> RED).
            a.raw(0xC7,0x05); a.blob(le32(cell('al_ret'))); a.blob(le32(FS_DATA_LO))   # mov [al_ret],FS_DATA_LO
        else:
            a.raw(0x89,0xC8); a.raw(0x05); a.blob(le32(FS_DATA_LO))  # eax=s ; add eax,FS_DATA_LO
            a.raw(0xA3); a.blob(le32(cell('al_ret')))             # [al_ret]=runstart
        # ---- free dir slot: first valid==0 slot -> fs_nent ----
        a.raw(0x31,0xC9)                                       # xor ecx,ecx (i=0)
        a.lbl('vput_slot')
        a.raw(0x83,0xF9,FS_D); a.j(JAE,'fs_put_reject')        # cmp i,FS_D ; jae reject (dir full)
        a.raw(0x6B,0xF1,FS_ENTSZ); a.raw(0x81,0xC6); a.absR('dirbuf')   # esi=dirbuf+i*28
        a.raw(0x83,0x3E,0x01); a.j(JNE,'vput_slotfound')       # cmp [esi+0],1 ; jne -> free slot
        a.raw(0x41); a.j(None,'vput_slot')
        a.lbl('vput_slotfound')
        a.raw(0x89,0x0D); a.blob(le32(cell('fs_nent')))        # [fs_nent]=slot
        # ---- multi-sector write loop: k=0..need-1 ----
        a.raw(0x31,0xC9)                                       # xor ecx,ecx (k=0)
        a.lbl('vput_wloop')
        a.raw(0x3B,0x0D); a.blob(le32(cell('al_size'))); a.j(JAE,'vput_wdone')   # cmp k,need ; jae done
        a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))           # [al_i]=k
        if mut!='nopadzero':
            a.raw(0xBF); a.absR('diskbuf')                     # edi=diskbuf
            a.raw(0x31,0xC0); a.raw(0xB9,0x80,0x00,0x00,0x00)  # xor eax,eax ; ecx=128
            a.raw(0xFC,0xF3,0xAB)                              # cld ; rep stosd (ZERO diskbuf -> no padding leak)
        # M-nopadzero FORGE: skip the zero -> the partial last sector leaks the PRIOR sector's stale bytes past the
        # payload (the raw-sector oracle reads [len%512 .. 512) and asserts ==0 -> non-zero -> RED).
        a.raw(0x8B,0x05); a.blob(le32(cell('al_i')))           # eax=k
        a.raw(0xC1,0xE0,0x09)                                  # shl eax,9 (k*512)
        a.raw(0xA3); a.blob(le32(cell('al_ptr')))             # [al_ptr]=offset
        a.raw(0x8B,0x05); a.blob(le32(cell('fs_len')))         # eax=len
        a.raw(0x2B,0x05); a.blob(le32(cell('al_ptr')))         # sub eax,offset -> remaining
        a.raw(0x3D); a.blob(le32(512)); a.j(JBE,'vput_chunkok')   # cmp remaining,512 ; jbe ok
        a.raw(0xB8,0x00,0x02,0x00,0x00)                        # mov eax,512
        a.lbl('vput_chunkok')
        a.raw(0xA3); a.blob(le32(cell('al_cur')))             # [al_cur]=chunklen
        a.raw(0x8B,0x35); a.blob(le32(cell('fs_payptr')))      # esi=payload_ptr
        a.raw(0x03,0x35); a.blob(le32(cell('al_ptr')))         # add esi,offset
        a.raw(0xBF); a.absR('diskbuf')                         # edi=diskbuf
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_cur')))         # ecx=chunklen
        a.raw(0xFC,0xF3,0xA4)                                  # cld ; rep movsb
        a.raw(0x8B,0x05); a.blob(le32(cell('al_ret')))         # eax=runstart
        a.raw(0x03,0x05); a.blob(le32(cell('al_i')))           # add eax,k
        a.raw(0xA3); a.blob(le32(cell('fs_lba')))             # [fs_lba]=runstart+k
        ata_write_sector('diskbuf', cell('fs_lba'))            # write sector + flush
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))           # ecx=k
        a.raw(0x41); a.j(None,'vput_wloop')                    # inc k ; loop
        a.lbl('vput_wdone')
        # ---- write dir entry slot[fs_nent] = {valid=1, len, name16, data_lba=runstart} + flush ----
        a.raw(0x8B,0x0D); a.blob(le32(cell('fs_nent')))        # ecx=slot
        a.raw(0x6B,0xF9,FS_ENTSZ); a.raw(0x81,0xC7); a.absR('dirbuf')   # edi=dirbuf+slot*28
        a.raw(0xC7,0x07,0x01,0x00,0x00,0x00)                   # mov dword[edi+0],1 (valid)
        a.raw(0x8B,0x05); a.blob(le32(cell('fs_len'))); a.raw(0x89,0x47,FS_OFF_LEN)   # [edi+4]=len
        a.raw(0x8B,0x05); a.blob(le32(cell('al_ret'))); a.raw(0x89,0x47,FS_OFF_LBA)   # [edi+24]=runstart
        a.raw(0x8B,0x35); a.blob(le32(cell('fs_nameptr')))     # esi=name_ptr
        a.raw(0x83,0xC7,FS_OFF_NAME)                           # add edi,8
        a.raw(0xB9,FS_NAMELEN,0x00,0x00,0x00)                  # ecx=16
        a.raw(0xFC,0xF3,0xA4)                                  # cld ; rep movsb (name)
        a.raw(0xC7,0x05); a.blob(le32(cell('tmp2'))); a.blob(le32(FS_DIR_LBA))
        ata_write_sector('dirbuf', cell('tmp2'))               # write dir + flush
        load_udata()
        ...
        ata_read_sector('dirbuf', cell('tmp2'))
        # (c)+(d) compute the target dir slot index (-> fs_nent) and its data_lba (-> fs_lba). The FROZEN delete PUT
        # allocates APPEND-BY-COUNT (slot = data_lba_off = count(valid==1)); backfill (link 43, fsreuse=True) switches this
        # to FIRST-FREE-SLOT (the LOWEST valid==0 hole), paying LEDGER D22 so a PUT after a NON-TAIL delete reuses the hole
        # instead of colliding with a live entry. (e)+(f) below are SHARED (they consume fs_nent + fs_lba), so fsreuse=False
        # is byte-identical to delete and only this block diverges.
        def _emit_put_count_alloc():
            # APPEND-BY-COUNT: nentries = count(valid==1) over EXACTLY D slots; slot = data_lba_off = nentries. The frozen
            # delete behaviour -- reproduced byte-for-byte when fsreuse=False (the parametrize-frozen-ref default).
            a.raw(0x31,0xC9)                                       # xor ecx,ecx  (i=0)
        ...
        if (not fsreuse) or mut=='append':
            # fsreuse=False -> the FROZEN delete PUT (byte-identical). M-append -> the D22 BUG inside the backfill kernel
            # (allocate by count -> collide with a live entry after a non-tail delete; the in-ref form of the frozen
            # differential).
            _emit_put_count_alloc()
        elif mut=='tailappend':
            # M-tailappend (FORGE): slot = (highest valid index)+1 -> never reuses a hole. In the capacity-exhausted forcing
            # (all D slots filled, then INTERIOR holes punched), the highest live index stays D-1, so tail = D -> reject ->
            # the new record is not stored -> RED.
            a.raw(0x31,0xC9)                                       # xor ecx,ecx  (i=0)
            a.raw(0x31,0xC0)                                       # xor eax,eax  (target = highest-valid-index + 1; default 0)
            a.lbl('fs_put_tscan')
            a.raw(0x83,0xF9,FS_D); a.j(JAE,'fs_put_tdone')         # cmp ecx,FS_D ; jae done
            a.raw(0x6B,0xF1,FS_ENTSZ)                              # imul esi,ecx,28
            a.raw(0x81,0xC6); a.absR('dirbuf')                     # add esi, dirbuf
            a.raw(0x83,0x3E,0x01); a.j(JNE,'fs_put_tnext')         # cmp [esi+0],1 ; jne skip (not valid)
            a.raw(0x8D,0x41,0x01)                                  # lea eax,[ecx+1]  (target = i+1)
            a.lbl('fs_put_tnext')
            a.raw(0x41); a.j(None,'fs_put_tscan')                  # inc ecx ; loop
            a.lbl('fs_put_tdone')
            a.raw(0xA3); a.blob(le32(cell('fs_nent')))             # mov [fs_nent], eax  (tail target slot)
            a.raw(0x83,0xF8,FS_D); a.j(JAE,'fs_put_reject')        # cmp eax,FS_D ; jae reject (tail past end)
            a.raw(0x05); a.blob(le32(FS_DATA_LO))                  # add eax, FS_DATA_LO
            a.raw(0xA3); a.blob(le32(cell('fs_lba')))              # mov [fs_lba], eax
        else:
            # GENUINE backfill: FIRST-FREE-SLOT scan -- the LOWEST valid==0 slot among ALL slots (the scan starts at i=0).
            # data_lba = FS_DATA_LO + freeslot (the frozen 1:1 invariant). No free slot (all D valid) -> reject. M-scanfrom1
            # starts the scan at i=1 (skips slot 0 -> a slot-0 hole is never reused -> the slot-0 forcing leg catches it);
            # M-wrongscan inverts the valid test (picks the first VALID slot -> overwrites a survivor); M-decoupled fixes
            # data_lba at FS_DATA_LO (the data sector is then a live survivor's -> the raw-SECTOR oracle catches it).
            if mut=='scanfrom1':
                a.raw(0xB9,0x01,0x00,0x00,0x00)                   # mov ecx,1  (FORGE: the scan skips slot 0)
            else:
                a.raw(0x31,0xC9)                                  # xor ecx,ecx  (i=0 -- scan from the LOWEST slot)
            a.lbl('fs_put_scan')
            a.raw(0x83,0xF9,FS_D); a.j(JAE,'fs_put_reject')        # cmp ecx,FS_D ; jae reject (no free slot -> directory full)
            a.raw(0x6B,0xF1,FS_ENTSZ)                              # imul esi,ecx,28
            a.raw(0x81,0xC6); a.absR('dirbuf')                     # add esi, dirbuf
            a.raw(0x83,0x3E,0x01)                                  # cmp dword[esi+FS_OFF_VALID(0)],1
            a.j(JE if mut=='wrongscan' else JNE,'fs_put_found')    # genuine: jne (valid!=1 -> FREE). M-wrongscan: je (first VALID -> overwrite a survivor).
            a.raw(0x41); a.j(None,'fs_put_scan')                   # inc ecx ; loop
            a.lbl('fs_put_found')
            a.raw(0x89,0x0D); a.blob(le32(cell('fs_nent')))        # mov [fs_nent], ecx  (the chosen free slot index)
            if mut=='decoupled':
                a.raw(0xC7,0x05); a.blob(le32(cell('fs_lba'))); a.blob(le32(FS_DATA_LO))   # mov [fs_lba], FS_DATA_LO (decoupled: wrong sector)
            else:
                a.raw(0x89,0xC8)                                   # mov eax,ecx
                a.raw(0x05); a.blob(le32(FS_DATA_LO))             # add eax, FS_DATA_LO  (data_lba = FS_DATA_LO + freeslot)
                a.raw(0xA3); a.blob(le32(cell('fs_lba')))         # mov [fs_lba], eax  (the data sector LBA)
    ...
    a.lbl('do_fs_get')
    if varsize:
        # ===== tract SYS_FS_GET (varsize): name-resolve -> MULTI-SECTOR reassemble, copy EXACTLY len =====
        load_kdata()
        a.raw(0x89,0x1D); a.blob(le32(cell('fs_nameptr')))     # [fs_nameptr]=ebx (query)
        a.raw(0x89,0x0D); a.blob(le32(cell('fs_payptr')))      # [fs_payptr]=ecx (dst)
        a.raw(0x89,0x15); a.blob(le32(cell('fs_len')))         # [fs_len]=edx (dst_cap)
        a.raw(0x8B,0x05); a.blob(le32(cell('cur')))            # eax=cur
        ld('esi',arr('alloc_lo'),'eax'); ld('edi',arr('alloc_hi'),'eax')
        a.raw(0x8B,0x1D); a.blob(le32(cell('fs_nameptr')))     # ebx=name_ptr
        a.raw(0x39,0xF3); a.j(JB,'fs_get_reject')
        a.raw(0x89,0xDA); a.raw(0x83,0xC2,FS_NAMELEN); a.j(JB,'fs_get_reject')   # edx=ebx+16 ; jc reject
        a.raw(0x39,0xFA); a.j(JA,'fs_get_reject')
        a.raw(0xC7,0x05); a.blob(le32(cell('tmp2'))); a.blob(le32(FS_DIR_LBA))
        ata_read_sector('dirbuf', cell('tmp2'))
        a.raw(0xC7,0x05); a.blob(le32(cell('fs_match'))); a.blob(le32(0))   # [fs_match]=0
        a.raw(0x31,0xC9)                                       # i=0
        a.lbl('vget_scan')
        a.raw(0x83,0xF9,FS_D); a.j(JAE,'vget_scandone')
        a.raw(0x89,0x0D); a.blob(le32(cell('fs_i')))           # [fs_i]=i
        a.raw(0x6B,0xF1,FS_ENTSZ); a.raw(0x81,0xC6); a.absR('dirbuf')   # esi=dirbuf+i*28
        a.raw(0x83,0x3E,0x01); a.j(JNE,'vget_scannext')        # cmp [esi+0],1 ; jne skip
        a.raw(0x8D,0x7E,FS_OFF_NAME)                           # lea edi,[esi+8]
        a.raw(0x89,0xFE)                                       # mov esi,edi  (esi->slot name)
        a.raw(0x8B,0x3D); a.blob(le32(cell('fs_nameptr')))     # edi=name_ptr (query)
        a.raw(0xB9,FS_NAMELEN,0x00,0x00,0x00)                  # ecx=16
        a.raw(0xFC,0xF3,0xA6)                                  # cld ; repe cmpsb
        a.raw(0x8B,0x0D); a.blob(le32(cell('fs_i')))           # ecx=i
        a.j(JNE,'vget_scannext')                               # names differ -> next
        a.raw(0x41); a.raw(0x89,0x0D); a.blob(le32(cell('fs_match')))   # inc ecx ; [fs_match]=i+1
        a.j(None,'vget_scandone')
        a.lbl('vget_scannext')
        a.raw(0x8B,0x0D); a.blob(le32(cell('fs_i'))); a.raw(0x41); a.j(None,'vget_scan')
        a.lbl('vget_scandone')
        a.raw(0x8B,0x05); a.blob(le32(cell('fs_match')))       # eax=match
        a.raw(0x85,0xC0); a.j(JE,'vget_notfound')              # 0 -> not found
        a.raw(0x48); a.raw(0x6B,0xF0,FS_ENTSZ); a.raw(0x81,0xC6); a.absR('dirbuf')   # dec eax ; esi=dirbuf+(match-1)*28
        a.raw(0x8B,0x46,FS_OFF_LEN)                            # eax=[esi+4] len
        a.raw(0x3D); a.blob(le32(TRACT_MAXLEN)); a.j(JA,'fs_get_reject')   # cmp len,2048 ; ja reject (corrupt)
        a.raw(0x3B,0x05); a.blob(le32(cell('fs_len'))); a.j(JA,'fs_get_reject')   # cmp len,dst_cap ; ja reject
        a.raw(0xA3); a.blob(le32(cell('fs_len')))             # [fs_len]=len (copy length)
        a.raw(0x05); a.blob(le32(511)); a.raw(0xC1,0xE8,0x09)  # need=(len+511)>>9
        a.raw(0xA3); a.blob(le32(cell('al_size')))            # [al_size]=need
        a.raw(0x8B,0x46,FS_OFF_LBA)                            # eax=[esi+24] data_lba (runstart)
        a.raw(0x3D); a.blob(le32(FS_DATA_LO)); a.j(JB,'fs_get_reject')   # cmp runstart,FS_DATA_LO ; jb reject
        a.raw(0xA3); a.blob(le32(cell('al_ret')))            # [al_ret]=runstart
        a.raw(0x03,0x05); a.blob(le32(cell('al_size')))       # eax=runstart+need (runend)
        if mut!='norunbound':
            a.j(JB,'fs_get_reject')                            # JC: data_lba+need WRAPPED (a corrupt near-4GiB data_lba) -> reject BEFORE the cmp (Codex caught: the cmp alone is not overflow-safe -- runend wraps to 0 and passes ja). OVERFLOW-SAFE run-window guard.
            a.raw(0x3D); a.blob(le32(TRACT_DATA_HI)); a.j(JA,'fs_get_reject')   # cmp runend,TRACT_DATA_HI ; ja reject (window straddle)
        # M-norunbound FORGE: drop the run-window straddle guard (carry-reject + the straddle cmp) -> a corrupt dir entry
        #   (data_lba near 4GiB -> wrap, or data_lba+need > window) reads PAST the FS window -> the hostile corrupt-entry leg leaks
        # (out-of-window kernel disk read). White-box: assert_varsize's win_sig (3D <TRACT_DATA_HI> 0F 87) absent -> caught.
        a.raw(0x8B,0x05); a.blob(le32(cell('cur')))           # access_ok(dst,len)
        ld('esi',arr('alloc_lo'),'eax'); ld('edi',arr('alloc_hi'),'eax')
        a.raw(0x8B,0x1D); a.blob(le32(cell('fs_payptr')))     # ebx=dst
        a.raw(0x39,0xF3); a.j(JB,'fs_get_reject')
        a.raw(0x8B,0x15); a.blob(le32(cell('fs_len')))        # edx=len
        a.raw(0x01,0xDA); a.j(JB,'fs_get_reject')             # add edx,ebx ; jc reject
        a.raw(0x39,0xFA); a.j(JA,'fs_get_reject')
        # ---- multi-sector read loop: k=0..need-1 ; copy min(512,len-k*512) diskbuf->dst+k*512 ----
        a.raw(0x31,0xC9)                                       # k=0
        a.lbl('vget_rloop')
        a.raw(0x3B,0x0D); a.blob(le32(cell('al_size'))); a.j(JAE,'vget_rdone')   # cmp k,need ; jae done
        a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))           # [al_i]=k
        a.raw(0x8B,0x05); a.blob(le32(cell('al_ret'))); a.raw(0x03,0x05); a.blob(le32(cell('al_i')))   # eax=runstart+k
        a.raw(0xA3); a.blob(le32(cell('fs_lba')))
        ata_read_sector('diskbuf', cell('fs_lba'))             # read sector
        a.raw(0x8B,0x05); a.blob(le32(cell('al_i'))); a.raw(0xC1,0xE0,0x09)   # eax=k*512
        a.raw(0xA3); a.blob(le32(cell('al_ptr')))             # [al_ptr]=offset
        if mut=='overcopy':
            # FORGE: copy 512 every sector (ignore len-offset) -> the last sector copies 512 -> writes PAST dst+len into
            # the module region (a >512 over-read). White-box: assert_varsize's GET chunk_sig (3D 00 02 00 00 0F 86) absent.
            a.raw(0xB8,0x00,0x02,0x00,0x00)                    # eax=512 (always)
        else:
            a.raw(0x8B,0x05); a.blob(le32(cell('fs_len'))); a.raw(0x2B,0x05); a.blob(le32(cell('al_ptr')))   # eax=len-offset
            a.raw(0x3D); a.blob(le32(512)); a.j(JBE,'vget_chunkok')
            a.raw(0xB8,0x00,0x02,0x00,0x00)                    # eax=512
            a.lbl('vget_chunkok')
        a.raw(0xA3); a.blob(le32(cell('al_cur')))             # [al_cur]=chunklen
        a.raw(0xBE); a.absR('diskbuf')                         # esi=diskbuf
        a.raw(0x8B,0x3D); a.blob(le32(cell('fs_payptr')))      # edi=dst
        a.raw(0x03,0x3D); a.blob(le32(cell('al_ptr')))         # add edi,offset
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_cur')))         # ecx=chunklen
        a.raw(0xFC,0xF3,0xA4)                                  # cld ; rep movsb (copy EXACTLY chunklen)
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); a.raw(0x41); a.j(None,'vget_rloop')   # k++ ; loop
        a.lbl('vget_rdone')
        a.raw(0x8B,0x0D); a.blob(le32(cell('fs_len')))         # ecx=len (returned)
        load_udata()
        a.raw(0xB8,0x01,0x00,0x00,0x00)                        # eax=1 (found)
        a.raw(0xCF)
        a.lbl('vget_notfound')
        load_udata()
        a.raw(0x31,0xC9); a.raw(0x31,0xC0); a.raw(0xCF)        # ecx=0 ; eax=0 ; iret
    else:
        load_kdata()
    ...
    # ---- SYS_ALLOC (eax==9): FIRST-FIT + split-on-alloc. EBX=size (the ONLY module input). Returns ptr (0=OOM) in eax,
    #      and the KERNEL emits 0xE0 + the 4 LE ptr bytes (the witness trace -- the module relays nothing). =====
    #   M-freenoop (companion FREE no-op): no chunk is ever reclaimed -> a re-alloc cannot reuse a freed span -> RED.
    #   M-nocoalesce (companion FREE no-merge): two adjacent freed spans never merge -> a re-alloc bigger than either
    #     alone (A5=80) OOMs -> RED.  M-nosplit: allocate the WHOLE chunk (never split) -> the next alloc OOMs -> RED.
    a.lbl('do_alloc')
    if mut=='bump':
        # M-bump FORGE (confused-deputy/correctness): a pure BUMP-POINTER allocator -- the free-list is NEVER consulted.
        # cursor (al_cur) lazy-inits to pool_base; alloc returns cursor + bumps it; OOM when cursor+size > pool end; FREE
        # is effectively dead (no reuse). On the late-bound witness the cumulative allocation >> pool, so without reuse
        # the cursor runs off the end and the FIRST reuse alloc OOMs -> the kernel-emitted ptr trace DIVERGES -> RED.
        load_kdata()
        a.raw(0x89,0x1D); a.blob(le32(cell('al_size')))   # al_size = ebx (size)
        mem(cell('al_cur')); a.raw(0x85,0xC0); a.j(JNE,'bump_have')      # al_cur==0 -> lazy-init to pool_base
        mem(cell('pool_base')); mme(cell('al_cur'))
        a.lbl('bump_have')
        mem(cell('al_cur')); a.raw(0x03,0x05); a.blob(le32(cell('al_size'))); a.raw(0x89,0xC2)  # edx = cur + size
        mem(cell('pool_base')); a.raw(0x03,0x05); a.blob(le32(cell('pool_size')))               # eax = pool end
        a.raw(0x39,0xC2); a.j(JA,'bump_oom')              # cmp edx,eax ; ja oom  (cur+size > end -> OOM)
        mem(cell('al_cur')); mme(cell('al_ret'))          # al_ret = cur
        mem(cell('al_cur')); a.raw(0x03,0x05); a.blob(le32(cell('al_size'))); mme(cell('al_cur'))  # cur += size
        a.j(None,'la_emit')
        a.lbl('bump_oom')
        a.raw(0xC7,0x05); a.blob(le32(cell('al_ret'))); a.blob(le32(0))  # al_ret = 0 (OOM)
        a.j(None,'la_emit')
    else:
        load_kdata()
        a.raw(0x89,0x1D); a.blob(le32(cell('al_size')))   # mov [al_size], ebx  (EBX is the ONLY module input)
        # SIZE reject (input validation; the durable stosd / cairn carry-check class). The MIN allocation is the 4-byte
        # SENTINEL WIDTH: SYS_DUMP reads a 4-byte sentinel at chunk.base (mov eax,[base]), so a live chunk with len<4
        # would make that kernel-side readback cross the chunk/pool boundary and EMIT the out-of-bounds bytes (a cross-
        # boundary read/leak; Codex caught it). Rejecting size<4 guarantees every live chunk has len>=4, so [base,base+4)
        # stays within the chunk -> within the pool. Also reject size>pool_size (no alloc can exceed the pool). WRAP-SAFETY
        # IS STRUCTURAL: size is validated <= pool_size and the pool is one small page, so base+size / base+len never
        # approach 2^32 (no carry). M-nosizewrap DROPS this floor -> alloc(0..3) builds a sub-sentinel USED chunk and
        # returns its nonzero base -> the hostile alloc0 / smallalloc legs emit a nonzero ptr where genuine emits 0 -> RED.
        # (size>pool_size also OOMs via the first-fit len-compare, so that half is defense-in-depth; the size<4 floor is
        # the output-forced discriminator.)
        if mut!='nosizewrap':
            a.raw(0x83,0xFB,0x04); a.j(JB,'la_oom')       # cmp ebx,4 ; jb la_oom   (size<4 -> reject; min-alloc = sentinel width)
            a.raw(0x3B,0x1D); a.blob(le32(cell('pool_size'))); a.j(JA,'la_oom')  # cmp ebx,[pool_size] ; ja la_oom (size>pool -> reject)
        if mut=='bestfit':
            # M-bestfit FORGE: BEST-fit instead of FIRST-fit -- scan ALL chunks and pick the SMALLEST free chunk with
            # len>=size (ties -> lowest address). On a free-list state with two holes where the lowest-address hole is
            # NOT the smallest, best-fit returns a DIFFERENT ptr than first-fit -> the emitted offset trace DIVERGES from
            # the host first-fit golden -> RED. (al_k = best index, al_cur = best len; both scratch in this arm -- do_free
            # is genuine and re-derives them.) The witness's E0 step (two holes 40>24, an alloc that fits both) forces this.
            a.raw(0xC7,0x05); a.blob(le32(cell('al_k'))); a.blob(le32(0xFFFFFFFF))    # best_i = none (-1)
            a.raw(0xC7,0x05); a.blob(le32(cell('al_cur'))); a.blob(le32(0xFFFFFFFF))  # best_len = max
            a.raw(0x31,0xC9)                                  # i=0
            a.lbl('bf_find')
            a.raw(0x3B,0x0D); a.blob(le32(cell('nchunks'))); a.j(JAE,'bf_done')       # cmp i,nchunks ; jae done
            a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))      # al_i = i
            ch_esi_from_ecx()                                 # esi -> chunk[i]
            a.raw(0x8B,0x46,0x08); a.raw(0x85,0xC0); a.j(JNE,'bf_next')               # used? -> skip
            a.raw(0x8B,0x46,0x04); a.raw(0x3B,0x05); a.blob(le32(cell('al_size'))); a.j(JB,'bf_next')  # len<size -> skip
            a.raw(0x8B,0x46,0x04); a.raw(0x3B,0x05); a.blob(le32(cell('al_cur'))); a.j(JAE,'bf_next')  # len>=best_len -> not better
            a.raw(0x8B,0x46,0x04); a.raw(0xA3); a.blob(le32(cell('al_cur')))          # best_len = len
            a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); a.raw(0x89,0x0D); a.blob(le32(cell('al_k')))   # best_i = i
            a.lbl('bf_next')
            a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); a.raw(0x41); a.j(None,'bf_find')   # i++ ; loop
            a.lbl('bf_done')
            a.raw(0x8B,0x0D); a.blob(le32(cell('al_k')))      # ecx = best_i
            a.raw(0x83,0xF9,0xFF); a.j(JE,'la_oom')           # cmp ecx,-1 ; je la_oom (no fit)
            a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))      # al_i = best_i
            a.j(None,'la_found')
        else:
            a.raw(0x31,0xC9)                                   # xor ecx,ecx  (i=0)
            a.lbl('la_find')
            a.raw(0x3B,0x0D); a.blob(le32(cell('nchunks'))); a.j(JAE,'la_oom')   # cmp ecx,[nchunks] ; jae OOM (no fit)
            a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))      # mov [al_i], ecx
            ch_esi_from_ecx()                                 # esi -> chunk[i]
            a.raw(0x8B,0x46,0x08); a.raw(0x85,0xC0); a.j(JNE,'la_next')          # used? jnz -> skip
            a.raw(0x8B,0x46,0x04); a.raw(0x3B,0x05); a.blob(le32(cell('al_size'))); a.j(JB,'la_next')  # len<size -> skip
            a.j(None,'la_found')
            a.lbl('la_next')
            a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); a.raw(0x41); a.j(None,'la_find')  # i++ ; loop
        a.lbl('la_found')
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))      # mov ecx,[al_i]
        ch_esi_from_ecx()
        a.raw(0x8B,0x46,0x04); a.raw(0x3B,0x05); a.blob(le32(cell('al_size'))); a.j(JE,'la_nosplit')  # len==size -> no split
        if mut!='nosplit':
            # CHUNK-TABLE-FULL guard: if the descriptor array is already at LARDER_MAXCHUNKS, REJECT (la_oom) rather than
            # split -- emit_larder_split's backward rep movsd would otherwise shift an entry into chunk[MAX], overrunning
            # the 16-entry array. M-nomaxchunks DROPS this guard -> the over-cap alloc proceeds -> the split overruns the
            # array -> corrupted/diverging output (caught by the table-fill hostile leg). Pinned in assert_larder.
            if mut!='nomaxchunks':
                mem(cell('nchunks')); a.raw(0x83,0xF8,LARDER_MAXCHUNKS); a.j(JAE,'la_oom')  # cmp nchunks,MAX ; jae OOM (table full)
            emit_larder_split()
        a.lbl('la_nosplit')
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))      # mov ecx,[al_i]
        ch_esi_from_ecx()
        a.raw(0xC7,0x46,0x08,0x01,0x00,0x00,0x00)         # chunk[i].used = 1
        a.raw(0x8B,0x06); a.raw(0xA3); a.blob(le32(cell('al_ret')))  # al_ret = chunk[i].base
        a.j(None,'la_emit')
        a.lbl('la_oom')
        a.raw(0xC7,0x05); a.blob(le32(cell('al_ret'))); a.blob(le32(0))   # al_ret = 0 (NULL on OOM / rejected size)
    a.lbl('la_emit')
    outi(0xE0)                                            # marker
    mem(cell('al_ret')); dr_eax()                         # emit the 4 LE ptr bytes (KERNEL-emitted trace)
    load_udata()
    mem(cell('al_ret'))                                   # eax = ptr (the syscall return value -- set LAST, after load_udata)
    a.raw(0xCF)

    # ---- SYS_FREE (eax==10): mark the chunk whose base==ECX free, then address-ordered coalesce with array neighbours
    #      (which ARE the address neighbours, since the array is gap-free). ECX is the ONLY module input. =====
    a.lbl('do_free')
    load_kdata()
    a.raw(0x89,0x0D); a.blob(le32(cell('al_ptr')))        # mov [al_ptr], ecx
    if mut=='freenoop':
        # M-freenoop FORGE: FREE never reclaims (the span stays used) -> a later alloc cannot reuse it -> the witness
        # diverges at A3 (which must reuse F(A1)'s freed span). Proves the reclaim/free-list is load-bearing.
        load_udata(); a.raw(0x31,0xC0); a.raw(0xCF)
    a.raw(0x31,0xC9)                                       # xor ecx,ecx (i=0)
    a.lbl('lf_find')
    a.raw(0x3B,0x0D); a.blob(le32(cell('nchunks'))); a.j(JAE,'lf_done')  # not found -> no-op
    a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))          # mov [al_i], ecx
    ch_esi_from_ecx()
    if mut=='nointeriorfree':
        # M-nointeriorfree FORGE (confused-deputy match-semantics): match a chunk by RANGE (base <= ptr < base+len)
        # instead of EXACT base. An interior-ptr free (ptr = a returned base + a nonzero offset) then frees the CONTAINING
        # chunk. The genuine exact-base match makes an interior free a clean no-op; this forge frees a live chunk -> the
        # live readback loses it -> RED. (The genuine production choice -- exact base, no module deref -- is what makes
        # interior / unowned / out-of-pool frees safe no-ops.)
        a.raw(0x8B,0x06); a.raw(0x3B,0x05); a.blob(le32(cell('al_ptr'))); a.j(JA,'lf_next')   # base > ptr -> skip (ptr < base)
        a.raw(0x8B,0x06); a.raw(0x03,0x46,0x04); a.raw(0x3B,0x05); a.blob(le32(cell('al_ptr'))); a.j(JBE,'lf_next')  # base+len <= ptr -> skip (ptr >= end)
    else:
        a.raw(0x8B,0x06); a.raw(0x3B,0x05); a.blob(le32(cell('al_ptr'))); a.j(JNE,'lf_next')   # EXACT: base!=ptr -> next (no module deref)
    a.raw(0x8B,0x46,0x08); a.raw(0x85,0xC0); a.j(JE,'lf_next')           # already free -> next (defensive; idempotent given full coalescing)
    a.raw(0xC7,0x46,0x08,0x00,0x00,0x00,0x00)             # chunk[i].used = 0
    a.j(None,'lf_coalesce')
    a.lbl('lf_next')
    a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); a.raw(0x41); a.j(None,'lf_find')   # i++ ; loop
    a.lbl('lf_coalesce')
    # NEXT-merge: if i+1 < nchunks and chunk[i+1].used==0 -> chunk[i].len += chunk[i+1].len ; delete i+1.
    #   M-nocoalesce drops BOTH merges; M-nonextmerge drops ONLY this one (closing the Codex gap that a prev-only
    #   coalescer passes a prev-only witness -- the A6=88 step now REQUIRES this next-merge).
    if mut not in ('nocoalesce','nonextmerge'):
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); a.raw(0x41)        # ecx = i+1
        a.raw(0x3B,0x0D); a.blob(le32(cell('nchunks'))); a.j(JAE,'lf_prev')   # i+1>=nchunks -> no next
        a.raw(0x89,0x0D); a.blob(le32(cell('al_k')))      # al_k = i+1 (delete candidate)
        ch_esi_from_ecx()                                 # esi -> chunk[i+1]
        a.raw(0x8B,0x46,0x08); a.raw(0x85,0xC0); a.j(JNE,'lf_prev')      # used? -> no merge
        if GH and mut=='nocrosspagecoalesce':
            # growheap M-nocrosspagecoalesce (FORGE): REFUSE to merge across a page boundary. The adjacency point of
            # chunk[i] and chunk[i+1] is chunk[i+1].base; if it is page-aligned ((base & PGSZ-1)==0) the two spans meet AT
            # a page boundary -> a within-page-only coalescer skips the merge. On the witness the make-or-break hole forms
            # by merging two freed spans that STRADDLE the boundary, so this forge OOMs E where genuine fits it -> RED.
            a.raw(0x8B,0x06)                              # mov eax,[esi]  (chunk[i+1].base = the adjacency boundary)
            a.raw(0x25); a.blob(le32(GROWHEAP_PGSZ-1))    # and eax, PGSZ-1
            a.j(JE,'lf_prev')                             # boundary page-aligned -> refuse cross-page next-merge
        a.raw(0x8B,0x46,0x04)                             # eax = chunk[i+1].len
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); ch_edi_from_ecx() # edi -> chunk[i]
        a.raw(0x01,0x47,0x04)                             # chunk[i].len += chunk[i+1].len
        emit_larder_delete()                              # delete index al_k (= i+1)
    a.lbl('lf_prev')
    # PREV-merge: if i>0 and chunk[i-1].used==0 -> chunk[i-1].len += chunk[i].len ; delete i.
    #   M-noprevmerge drops ONLY this one (the A5=80 step REQUIRES it).
    if mut not in ('nocoalesce','noprevmerge'):
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))      # ecx = i
        a.raw(0x85,0xC9); a.j(JE,'lf_done')               # i==0 -> done
        a.raw(0x49)                                       # ecx = i-1
        ch_esi_from_ecx()                                 # esi -> chunk[i-1]
        a.raw(0x8B,0x46,0x08); a.raw(0x85,0xC0); a.j(JNE,'lf_done')      # prev used? -> done
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))      # ecx = i
        a.raw(0x89,0x0D); a.blob(le32(cell('al_k')))      # al_k = i (delete index i after merge)
        ch_edi_from_ecx()                                 # edi -> chunk[i]  (esi still -> chunk[i-1])
        if GH and mut=='nocrosspagecoalesce':
            # growheap M-nocrosspagecoalesce (FORGE), prev-merge arm: the adjacency point of chunk[i-1] and chunk[i] is
            # chunk[i].base; if page-aligned, REFUSE the merge. THIS is the arm the witness bites: free(C) (chunk[i]=C at
            # base pool_base+PGSZ, page-aligned) tries to prev-merge with the freed B below it; the genuine merges them
            # into the boundary-straddling hole that E needs, the forge refuses -> E OOMs -> the emitted trace DIVERGES.
            a.raw(0x8B,0x07)                              # mov eax,[edi]  (chunk[i].base = the adjacency boundary)
            a.raw(0x25); a.blob(le32(GROWHEAP_PGSZ-1))    # and eax, PGSZ-1
            a.j(JE,'lf_done')                             # boundary page-aligned -> refuse cross-page prev-merge
        a.raw(0x8B,0x47,0x04)                             # eax = chunk[i].len
        a.raw(0x01,0x46,0x04)                             # chunk[i-1].len += chunk[i].len
        emit_larder_delete()                              # delete index al_k (= i)
    a.lbl('lf_done')
    load_udata()
    a.raw(0x31,0xC0)                                       # eax=0
    a.raw(0xCF)

    # ---- SYS_DUMP (eax==11): the KERNEL walks the live chunks in ADDRESS order and emits, framed by 0xE1..0xE2, for each
    #      live chunk: 4 LE ptr bytes + the 4 LE bytes of [ptr] (the sentinel the module wrote through that ptr). =====
    a.lbl('do_dump')
    load_kdata()
    outi(0xE1)                                            # dump-begin marker
    a.raw(0x31,0xC9)                                       # xor ecx,ecx (i=0)
    a.lbl('ld_loop')
    a.raw(0x3B,0x0D); a.blob(le32(cell('nchunks'))); a.j(JAE,'ld_done')
    a.raw(0x89,0x0D); a.blob(le32(cell('al_i')))          # mov [al_i], ecx
    ch_esi_from_ecx()
    a.raw(0x8B,0x46,0x08); a.raw(0x85,0xC0); a.j(JE,'ld_next')   # not used -> skip
    a.raw(0x8B,0x06); dr_eax()                            # emit chunk[i].base (4 bytes)
    a.raw(0x8B,0x06); a.raw(0x8B,0x00); dr_eax()          # mov eax,[base] ; mov eax,[eax] ; emit the sentinel dword
    a.lbl('ld_next')
    a.raw(0x8B,0x0D); a.blob(le32(cell('al_i'))); a.raw(0x41); a.j(None,'ld_loop')   # i++ ; loop
    a.lbl('ld_done')
    outi(0xE2)                                            # dump-end marker
    load_udata()
    a.raw(0x31,0xC0)
    a.raw(0xCF)

    # ===== link45 (highwater): SYS_FALLOC (eax==13) + SYS_HWDUMP (eax==14) =====
    # SYS_FALLOC: allocate a fresh physical frame TOP-DOWN from the author-unknown region_hi (the RAM ceiling the boot mmap
    # scan discovered -- author-unknown because the gate boots at a per-run-random -m), map it NON-IDENTITY at the reused
    # lethe alias V (PTE[V]<-frame|7 ; invlpg [V]), record it in hw_frames[] (the inert chunk[] block), and KERNEL-EMIT its
    # physical address (0xF0 ++ le32(frame)). The prober then writes a seed-derived payload THROUGH V (CPL3). The kernel
    # emits the address (provenance: a baked-address forge emits the wrong addr for a random -m -> RED). Scratch reused:
    # al_cur=the descending cursor, al_i=the alloc count, al_ptr=the emitted (top-down) frame, tmp0=the actually-mapped frame
    # (== al_ptr in the genuine build; differs only for M-hwsingleframe).
    if highwater:
        a.lbl('do_falloc')
        load_kdata()
        # HARD per-boot cap: if the alloc count has reached HW_MAXFRAMES, reject (OOM). Bounds the hw_frames[] index (no CPL0
        # write past the array) AND the cursor descent (<=HW_MAXFRAMES frames below region_hi -- a hostile CPL3 loop can never
        # descend into the kernel/page-table/proc region and alias it User|RW). M-hwnocap drops it -> a hostile over-cap loop
        # overruns hw_frames[] / descends below the safe window -> the gate's hostile leg catches it.
        if mut!='hwnocap':
            a.raw(0x8B,0x05); a.blob(le32(cell('al_i')))                 # eax = count
            a.raw(0x3D); a.blob(le32(HW_MAXFRAMES)); a.j(JAE,'hw_oom')   # cmp count,HW_MAXFRAMES ; jae OOM
        if mut=='hwbumpup':
            # M-hwbumpup: allocate BOTTOM-UP from a fixed low base (author-KNOWN addresses) -> host expects top-down -> RED.
            mem(cell('al_cur')); a.raw(0x85,0xC0); a.j(JNE,'hw_have')
            a.raw(0xB8); a.blob(le32(0x400000)); mme(cell('al_cur'))   # cursor init = 0x400000 (low, author-known)
            a.lbl('hw_have')
            mem(cell('al_cur')); mme(cell('al_ptr'))                   # frame = cursor
            mem(cell('al_cur')); a.raw(0x05); a.blob(le32(0x1000)); mme(cell('al_cur'))  # cursor += 0x1000
        else:
            # GENUINE: cursor starts at region_hi (author-unknown), descends one frame per call.
            mem(cell('al_cur')); a.raw(0x85,0xC0); a.j(JNE,'hw_have')  # if al_cur==0: init
            mem(cell('region_hi')); mme(cell('al_cur'))               #   al_cur = region_hi (page-aligned ceiling)
            a.lbl('hw_have')
            mem(cell('al_cur')); a.raw(0x2D); a.blob(le32(0x1000))    # eax = al_cur - 0x1000 = frame
            a.raw(0x3B,0x05); a.blob(le32(cell('region_lo'))); a.j(JB,'hw_oom')  # cmp frame,region_lo ; jb OOM
            mme(cell('al_cur')); mme(cell('al_ptr'))                  # al_cur=frame (descend) ; al_ptr=frame
        # choose the frame to MAP+STORE: genuine = the top-down al_ptr ; M-hwsingleframe = a FIXED baked frame (addresses
        # still pass, but every readback collapses to the last payload -> RED on the distinctness witness).
        if mut=='hwsingleframe':
            a.raw(0xB8); a.blob(le32(F_FRAME))                        # eax = F_FRAME (baked, same every call)
        else:
            mem(cell('al_ptr'))                                       # eax = frame (genuine, distinct per call)
        mme(cell('tmp0'))                                            # tmp0 = map_frame
        a.raw(0x83,0xC8,0x07)                                        # eax = map_frame|7  (Present+RW+User)
        a.raw(0xA3); a.absR('pt',(V_VADDR>>12)*4)                    # mov [PTE_V], eax   (NON-IDENTITY alias install)
        if mut!='hwnoinvlpg':
            a.raw(0x0F,0x01,0x3D); a.blob(le32(V_VADDR))             # invlpg [V]  (targeted; lethe)
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_i')))                # ecx = count
        a.raw(0x8B,0x15); a.blob(le32(cell('tmp0')))               # edx = map_frame
        st(chunk_base(),'ecx','edx')                                # hw_frames[count] = map_frame
        a.raw(0xFF,0x05); a.blob(le32(cell('al_i')))               # count++
        if mut=='hwbakedaddr':
            outi(HW_AMARK); a.raw(0xB8); a.blob(le32(0x12340000)); dr_eax()   # M-hwbakedaddr: emit a FIXED addr -> RED on address
        else:
            outi(HW_AMARK); mem(cell('al_ptr')); dr_eax()           # emit 0xF0 ++ le32(frame)  (the top-down/bumpup addr)
        load_udata()
        mem(cell('al_ptr'))                                          # eax = frame (return to the prober; CPL0 reads the kernel cell)
        a.raw(0xCF)                                                 # iret -> CPL3
        a.lbl('hw_oom')
        outi(HW_AMARK); a.raw(0x31,0xC0); dr_eax()                 # OOM: emit 0xF0 ++ 0
        load_udata(); a.raw(0x31,0xC0); a.raw(0xCF)

        # SYS_HWDUMP: for each recorded frame, re-map V<-frame (+invlpg), read [V] back (the payload the prober wrote when V
        # mapped that frame), and KERNEL-EMIT addr ++ readback. Distinctness: distinct frames -> distinct payloads. A single
        # baked frame (M-hwsingleframe) -> all readbacks == the last payload -> RED. No invlpg (M-hwnoinvlpg) -> the stale
        # V->lastframe TLB entry serves every read -> all readbacks == the last payload -> RED (invlpg is load-bearing).
        a.lbl('do_hwdump')
        load_kdata()
        outi(HW_DBEGIN)
        a.raw(0x31,0xC9)                                            # ecx = 0 (j)
        a.lbl('hw_dloop')
        a.raw(0x3B,0x0D); a.blob(le32(cell('al_i'))); a.j(JAE,'hw_ddone')  # cmp j,count ; jae done
        a.raw(0x89,0x0D); a.blob(le32(cell('al_k')))              # al_k = j (survive the work)
        ld('eax',chunk_base(),'ecx')                               # eax = hw_frames[j]
        mme(cell('al_ptr'))                                        # al_ptr = frame
        a.raw(0x83,0xC8,0x07)                                      # eax = frame|7
        a.raw(0xA3); a.absR('pt',(V_VADDR>>12)*4)                  # mov [PTE_V], frame|7
        if mut!='hwnoinvlpg':
            a.raw(0x0F,0x01,0x3D); a.blob(le32(V_VADDR))           # invlpg [V]
        a.raw(0xA1); a.blob(le32(V_VADDR))                         # eax = [V]  (readback through the alias)
        mme(cell('al_ret'))                                        # al_ret = readback
        mem(cell('al_ptr')); dr_eax()                             # emit le32(frame)
        mem(cell('al_ret')); dr_eax()                             # emit le32(readback)
        a.raw(0x8B,0x0D); a.blob(le32(cell('al_k'))); a.raw(0x41) # ecx = al_k ; j++
        a.j(None,'hw_dloop')
        a.lbl('hw_ddone')
        outi(HW_DEND)
        load_udata()
        a.raw(0x31,0xC0); a.raw(0xCF)
    ...
    if fsdel:
        # ---- SYS_FS_DEL (eax==12): tombstone a NAMED record IN PLACE (mutable-FS DELETE -- THE LINK). =====
        #   ABI: EBX=name_ptr (16 bytes, the name to delete). Returns eax=1 (deleted) / 0 (not found).
        #   Placed AFTER the timer/gp/pf/panic handlers (after assert_cairn's get_arm window delimiter -- the timer_sig --
        #   so assert_cairn's per-arm cld counts are untouched) and using a MINIMAL ebx-only prologue (NOT the 3-save
        #   `mov[fs_nameptr],ebx;mov[fs_payptr],ecx;mov[fs_len],edx`) so assert_cairn still finds EXACTLY 2 fs_prologues.
        #   The kernel (CPL0):
        #     (a) access_ok(name_ptr,16) against [alloc_lo,alloc_hi)[cur] -- the same carry-guarded check GET/PUT use
        #         (M-nocarrycheck drops the carry jb -> a near-4GiB name_ptr+16 WRAPs small + passes -> out-of-region read).
        #     (b) read FS_DIR_LBA into dirbuf; FIXED-loop scan EXACTLY FS_D slots for valid==1 && the full 16-byte name ==
        #         the query (cld;repe cmpsb -- a 16-byte compare, NOT a prefix scan; M-positional IGNORES the name and
        #         takes the FIRST valid slot -> deletes the wrong record).
        #     (c) on match: tombstone valid:=0 IN PLACE (M-noop SKIPS it -> nothing deleted; M-wipeall clears EVERY slot ->
        #         over-deletes the survivor), then ATA-WRITE dirbuf back to FS_DIR_LBA + CACHE FLUSH (M-noflush drops the
        #         flush -> the tombstone never reaches the medium -> lost across the reboot). iret eax=1.
        #     on no match / reject: iret eax=0. DELETE adds no new DIRECT attacker-influenced read/write (it writes a FIXED
        #     LBA + access_ok's its only module ptr -- the same surface as cairn's name access_ok + dir write-back). It DOES
        #     introduce a CROSS-OPERATION surface (Codex-flagged): a tombstone leaves a HOLE in the dir, and the FROZEN PUT
        #     allocator (slot/data_lba = count(valid==1), append-by-count) would CORRUPT a live entry if a PUT follows a
        #     NON-TAIL delete. That is a CONSCIOUSLY-DEFERRED LEDGER debt (no landed program does PUT-after-DELETE; the
        #     forcing here never does); the next delete+create/reuse link must move PUT to FIRST-FREE-SLOT (param-gated).
        a.lbl('do_fs_del')
        load_kdata()
        a.raw(0x89,0x1D); a.blob(le32(cell('fs_nameptr')))     # mov [fs_nameptr], ebx (the ONLY module input -- minimal prologue)
        # (a) access_ok(name_ptr, 16).
        a.raw(0x8B,0x05); a.blob(le32(cell('cur')))            # eax = cur
        ld('esi',arr('alloc_lo'),'eax'); ld('edi',arr('alloc_hi'),'eax')
        a.raw(0x8B,0x1D); a.blob(le32(cell('fs_nameptr')))     # ebx = name_ptr
        a.raw(0x39,0xF3); a.j(JB,'fs_del_reject')              # cmp ebx,esi(lo) ; jb reject
        a.raw(0x89,0xDA)                                       # mov edx,ebx
        a.raw(0x83,0xC2,FS_NAMELEN)                            # add edx,16
        if mut!='nocarrycheck': a.j(JB,'fs_del_reject')        # JC reject (CARRY = name_ptr+16 WRAP -> out-of-region; inherited access_ok-overflow guard). M-nocarrycheck drops it.
        a.raw(0x39,0xFA); a.j(JA,'fs_del_reject')              # cmp edx,edi(hi) ; ja reject
        # (b) read the directory, FIXED-loop scan D slots for valid && name==query.
        a.raw(0xC7,0x05); a.blob(le32(cell('tmp2'))); a.blob(le32(FS_DIR_LBA))   # mov [tmp2], FS_DIR_LBA
        ata_read_sector('dirbuf', cell('tmp2'))
        a.raw(0xC7,0x05); a.blob(le32(cell('fs_match'))); a.blob(le32(0))        # mov [fs_match], 0 (slot index+1 of match; 0 == no match)
        a.raw(0x31,0xC9)                                       # xor ecx,ecx  (i=0)
        a.lbl('fs_del_scan')
        a.raw(0x83,0xF9,FS_D); a.j(JAE,'fs_del_scandone')      # cmp ecx,FS_D ; jae done (FIXED loop, exactly D slots)
        a.raw(0x89,0x0D); a.blob(le32(cell('fs_i')))           # mov [fs_i], ecx  (save i across the compare)
        a.raw(0x6B,0xF1,FS_ENTSZ)                              # imul esi,ecx,28
        a.raw(0x81,0xC6); a.absR('dirbuf')                     # add esi, dirbuf
        a.raw(0x83,0x3E,0x01); a.j(JNE,'fs_del_scannext')      # cmp dword[esi+0(valid)],1 ; jne skip (not live)
        if mut != 'positional':
            # full 16-byte name compare: esi -> slot name (esi+8), edi = name_ptr (query). rep cmpsb 16.
            a.raw(0x8D,0x7E,FS_OFF_NAME)                       # lea edi,[esi+8]   (slot name field)
            a.raw(0x89,0xFE)                                   # mov esi,edi       (esi -> slot name)
            a.raw(0x8B,0x3D); a.blob(le32(cell('fs_nameptr'))) # mov edi, name_ptr (query name)
            a.raw(0xB9,FS_NAMELEN,0x00,0x00,0x00)              # mov ecx,16
            if mut!='fsnocld': a.raw(0xFC)                     # cld  (DEL name-compare; M-fsnocld drops it -> a hostile std=DF=1 makes repe cmpsb walk BACKWARD off the dir slot/query into kernel memory -> wrong match)
            a.raw(0xF3,0xA6)                                   # repe cmpsb  (ZF=1 iff fully equal)
            a.raw(0x8B,0x0D); a.blob(le32(cell('fs_i')))       # mov ecx, [fs_i]  (restore i)
            a.j(JNE,'fs_del_scannext')                         # names differ -> next slot
        else:
            # M-positional (FORGE): ignore the query name -> the FIRST valid slot is the "match" (positional, NOT by name).
            # With ALPHA put FIRST (slot 0, the SURVIVOR) + BRAVO second (slot 1, the delete-target), deleting BRAVO by
            # name = slot 1, but M-positional deletes slot 0 (ALPHA) -> BOTH BOOT-3 queries diverge -> RED.
            a.raw(0x8B,0x0D); a.blob(le32(cell('fs_i')))       # mov ecx,[fs_i]  (i, so the fall-through records this slot)
        # MATCH: record slot index (i+1 so 0 == no-match) and break (first match wins).
        a.raw(0x41)                                            # inc ecx (i+1)
        a.raw(0x89,0x0D); a.blob(le32(cell('fs_match')))       # mov [fs_match], i+1
        a.j(None,'fs_del_scandone')                            # first match wins -> stop scanning
        a.lbl('fs_del_scannext')
        a.raw(0x8B,0x0D); a.blob(le32(cell('fs_i')))           # mov ecx, [fs_i]
        a.raw(0x41); a.j(None,'fs_del_scan')                   # inc i ; loop
        a.lbl('fs_del_scandone')
        a.raw(0x8B,0x05); a.blob(le32(cell('fs_match')))       # eax = match (i+1, or 0 if none)
        a.raw(0x85,0xC0); a.j(JE,'fs_del_notfound')            # test eax,eax ; jz -> not found
        if mut != 'noop':
            if mut == 'wipeall':
                # M-wipeall (FORGE): tombstone EVERY slot -> the SURVIVOR(ALPHA) is also cleared -> BOOT-3 GET(ALPHA) -> not-found -> RED.
                a.raw(0x31,0xC9)                               # xor ecx,ecx
                a.lbl('fs_del_wipe')
                a.raw(0x83,0xF9,FS_D); a.j(JAE,'fs_del_wipedone')   # cmp ecx,FS_D ; jae done
                a.raw(0x6B,0xF9,FS_ENTSZ)                       # imul edi,ecx,28
                a.raw(0x81,0xC7); a.absR('dirbuf')             # add edi, dirbuf
                a.raw(0xC7,0x07,0x00,0x00,0x00,0x00)           # mov dword[edi+0(valid)],0
                a.raw(0x41); a.j(None,'fs_del_wipe')           # inc ecx ; loop
                a.lbl('fs_del_wipedone')
            else:
                # GENUINE tombstone: recompute the matched slot base (i = match-1), set valid:=0 IN PLACE. The recompute
                # (dec eax; imul esi,eax,28; add esi,dirbuf) BINDS the tombstone to the matched-slot dataflow -- assert_delete
                # pins this contiguous block so a dead C7 06 elsewhere cannot satisfy the pin.
                a.raw(0x48)                                    # dec eax (i)
                a.raw(0x6B,0xF0,FS_ENTSZ)                      # imul esi,eax,28
                a.raw(0x81,0xC6); a.absR('dirbuf')            # add esi, dirbuf  (esi -> matched slot)
                if mut == 'corruptlen':
                    # M-corruptlen (FORGE, OUTPUT-INVISIBLE): clear the matched slot's LEN (esi+4) instead of its VALID
                    # (esi+0). GET then finds valid==1 + a full name match + len==0 -> copies 0 bytes -> emits NOTHING, so
                    # grade_fs_deleted reads "deleted" (FALSE -- the record is NOT tombstoned, valid still 1). The 3-phase
                    # output grades GREEN; this forge is caught ONLY by assert_delete (no C7 06 clearing valid at offset 0).
                    a.raw(0xC7,0x46,0x04,0x00,0x00,0x00,0x00) # mov dword[esi+4(len)],0
                else:
                    a.raw(0xC7,0x06,0x00,0x00,0x00,0x00)      # mov dword[esi+0(valid)],0  -- THE TOMBSTONE (M-noop drops this whole branch)
        # ATA WRITE dirbuf back to FS_DIR_LBA + CACHE FLUSH (M-noflush drops the flush via ata_write_sector(flush=False)).
        a.raw(0xC7,0x05); a.blob(le32(cell('tmp2'))); a.blob(le32(FS_DIR_LBA))   # mov [tmp2], FS_DIR_LBA
        ata_write_sector('dirbuf', cell('tmp2'), flush=(mut!='noflush'))
        load_udata()
        a.raw(0xB8,0x01,0x00,0x00,0x00)                        # eax=1 (deleted)
        a.raw(0xCF)
        a.lbl('fs_del_notfound')
        load_udata()
        a.raw(0x31,0xC0)                                       # eax=0 (not found)
        a.raw(0xCF)
        a.lbl('fs_del_reject')
    ...
    if varsize:
        # tract: the TRACT_W-byte free-map for the recompute-from-directory first-fit allocator. Emitted ONLY under
        # varsize (gated) so frozen modes are byte-identical; placed at the image TAIL so nothing before it shifts.
        a.lbl('fsused'); a.blob(b'\x00'*TRACT_W)
    a.lbl('body_start')
...
def build_elf(mut=None, stage='full', npages=1, fsdel=False, fsreuse=False, varsize=False, highwater=False):   # +link45 highwater (False == tract byte-for-byte)
    code0,_=build_code(0,0,mut,stage,npages,fsdel,fsreuse,varsize,highwater); clen=len(code0)
    memsz=12+clen+16384
    kstack=LOAD+memsz; kend=LOAD+memsz
    code,labels=build_code(kstack,kend,mut,stage,npages,fsdel,fsreuse,varsize,highwater); assert len(code)==clen,(len(code),clen)
...
# ============================ link45 (highwater) ring-3 prober (proc0, K=1) ============================
def module_highwater_prober(seed=None, n=None):
    # The highwater prober. With seed=None it reads a LATE-BOUND seed byte over COM1 (SYS_READ); with seed given it BAKES it
    # (STEP-0 smoke). It calls SYS_FALLOC n times -- after each, the kernel maps the fresh top-down frame at V (+invlpg) and
    # emits the address; the prober writes a seed-derived payload P_i = HW_TAG|((seed&0xFF)<<8)|i THROUGH V (-> frame_i) --
    # then SYS_HWDUMP (the kernel re-maps each frame at V + invlpg + reads back + emits addr++payload) and SYS_EXIT.
    if n is None: n=HW_N
    m=Asm()
    if seed is None:
        m.raw(0xB8,0,0,0,0); m.raw(0xCD,0x30); m.raw(0x25); m.blob(le32(0xFF))   # eax=0(SYS_READ); int 0x30 -> seed; and eax,0xFF
    else:
        m.raw(0xB8); m.blob(le32(seed & 0xFF))                                    # mov eax, seed (baked)
    m.raw(0x89,0xC3)                              # mov ebx,eax  (seed survives our int 0x30s -- the kernel arms set only eax)
    for i in range(n):
        m.raw(0xB8); m.blob(le32(SYS_FALLOC)); m.raw(0xCD,0x30)   # eax=13; int 0x30 -> kernel maps V<-frame_i (+invlpg) + emits addr
        m.raw(0x89,0xDA)                          # mov edx,ebx (seed)
        m.raw(0x81,0xE2); m.blob(le32(0xFF))      # and edx,0xFF
        m.raw(0xC1,0xE2,0x08)                     # shl edx,8
        m.raw(0x81,0xCA); m.blob(le32(HW_TAG | (i & 0xFF)))   # or edx, HW_TAG|i  -> P_i
        m.raw(0x89,0x15); m.blob(le32(V_VADDR))   # mov [V_VADDR], edx   (write P_i THROUGH the alias -> frame_i)
    m.raw(0xB8); m.blob(le32(SYS_HWDUMP)); m.raw(0xCD,0x30)   # eax=14; int 0x30 -> kernel readback+emit (distinctness witness)
    m.raw(0xB3,0x00); m.raw(0xB8); m.blob(le32(SYS_EXIT)); m.raw(0xCD,0x30)   # bl=0; eax=1; int 0x30 (SYS_EXIT)
    m.raw(0xEB,0xFE)                              # hang (kernel finalizes on live==0)
    return m.assemble()[0]

def hw_region_hi_qemu(ram_mb): return ram_mb*0x100000 - 0x20000   # QEMU -kernel mmap top (validated TCG==KVM, 24M..192M)
def hw_expect(ram_mb, seed, n=None):
    # the host oracle's expectation: N frames TOP-DOWN from region_hi, each holding its seed-derived payload.
    if n is None: n=HW_N
    rh=hw_region_hi_qemu(ram_mb)
    frames=[(rh-(i+1)*0x1000) & 0xFFFFFFFF for i in range(n)]
    pays=[hw_payload(seed,i) for i in range(n)]
    return frames, pays
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'highwater', cd['nprocs'])
...
# ============================ link40 (larder) WHITE-BOX assert ============================
def assert_larder(kelf):
    """White-box structural co-pin for larder's heap allocator (the LATE-BOUND output grade in larder_latebound.py is the
       PRIMARY binding; this co-pin pins the structure an output forge could try to fake, and the input-validation/
       confused-deputy pieces that are output-invisible on the benign witness). All pins are ARM-LOCAL (bounded to the
       do_alloc / do_free / do_dump arms) and use the pin-reachability-not-presence doctrine (branch TARGETs, not mere
       opcode presence). The cell addresses are larder_ref's own (fixed layout); the arms are located WITHIN the passed
       kelf so a mutant's shifted arm is graded in place. Pins:
         (1) ABI: do_alloc reads ONLY EBX (its prologue is load_kdata ; mov [al_size],ebx); do_free reads ONLY ECX (its
             prologue is load_kdata ; mov [al_ptr],ecx).
         (2) SIZE reject, branch TARGET = la_oom: `cmp ebx,4 ; jb la_oom` (min-alloc = the 4-byte sentinel width, so the
             SYS_DUMP readback [base,base+4) stays in-chunk) AND `cmp ebx,[pool_size] ; ja la_oom`, both present, the
             jb/ja rel32s decoding to la_oom (the al_ret=0 reject). M-nosizewrap drops them -> REJECT.
         (3) the FIRST-FIT search loop: `cmp ecx,[nchunks] ; jae la_oom` whose jae also targets la_oom (the search reaches
             OOM, not a dead fall-through).
         (3b) the CHUNK-TABLE-FULL guard `mov eax,[nchunks] ; cmp eax,LARDER_MAXCHUNKS ; jae la_oom` (an at-cap alloc is
             rejected before emit_larder_split can overrun chunk[MAX]); the jae targets la_oom. M-nomaxchunks drops it -> REJECT.
         (4) the SPLIT-remainder insert EXACTLY ONCE in do_alloc (std;rep movsd;cld = FD F3 A5 FC). M-nosplit -> 0 -> REJECT.
         (5) the kernel-emit path: outi(0xE0) in do_alloc; the 0xE1..0xE2 dump frame in do_dump (the module relays nothing).
         (6) do_free EXACT base match (mov eax,[esi] ; cmp eax,[al_ptr] ; jne), NOT a range match -- M-nointeriorfree
             replaces it -> REJECT.
         (7) the prev+next COALESCE deletes EXACTLY TWICE in do_free (cld;rep movsd = FC F3 A5). M-nocoalesce -> 0,
             M-noprevmerge/M-nonextmerge -> 1 -> REJECT.
       Returns True/False. Mirrors assert_cairn / assert_durability."""
    le = lambda v: struct.pack('<I', v & 0xFFFFFFFF)
    lk = bytes([0xB8])+le(KDATA)+bytes([0x8E,0xD8,0x8E,0xC0,0x8E,0xE0,0x8E,0xE8])   # load_kdata
    alloc_anchor = lk + bytes([0x89,0x1D])+le(cell('al_size'))    # do_alloc prologue: load_kdata ; mov [al_size],ebx
    free_anchor  = lk + bytes([0x89,0x0D])+le(cell('al_ptr'))     # do_free prologue:  load_kdata ; mov [al_ptr],ecx
    dump_anchor  = lk + bytes([0xB0,0xE1,0xE6,OUT])               # do_dump prologue:  load_kdata ; out 0xE1 (dump-begin)
    sa = kelf.find(alloc_anchor)
    if sa < 0: return False                                       # do_alloc arm absent (or ABI changed) -> REJECT
    sf = kelf.find(free_anchor)
    if sf < 0 or sf <= sa: return False                           # do_free arm absent / out of order -> REJECT
    sd = kelf.find(dump_anchor, sf)
    if sd < 0: return False                                       # do_dump arm absent -> REJECT
    alloc_arm = kelf[sa:sf]
    free_arm  = kelf[sf:sd]
    dump_arm  = kelf[sd:sd+256]
    # (1) the ABI is pinned by the anchors themselves (the FIRST data op after load_kdata is the ebx / ecx save).
    # locate la_oom (the al_ret=0 reject) in the alloc arm -- the shared target of every reject branch.
    la_oom_blk = bytes([0xC7,0x05])+le(cell('al_ret'))+bytes([0,0,0,0])   # mov dword[al_ret],0
    rj = alloc_arm.find(la_oom_blk)
    if rj < 0: return False
    def target_is_oom(jcc_at):                                    # jcc_at = arm offset of the 0F 8x opcode (6-byte near jcc)
        if alloc_arm[jcc_at] != 0x0F: return False
        rel = int.from_bytes(alloc_arm[jcc_at+2:jcc_at+6], 'little', signed=True)
        return (jcc_at + 6 + rel) == rj
    # (2) the size reject, branch TARGET = la_oom (pin_reachability_not_presence). The floor is `cmp ebx,4 ; jb la_oom`
    # (min-alloc = the 4-byte sentinel width -- closes the SYS_DUMP cross-boundary readback edge) THEN `cmp ebx,[pool_size]
    # ; ja la_oom`. (Updated from the earlier `test ebx,ebx ; jz` size==0 floor when the floor was raised to 4.)
    cmp_floor = bytes([0x83,0xFB,0x04,0x0F,0x82])                 # cmp ebx,4 ; jb rel32
    cmp_psz = bytes([0x3B,0x1D])+le(cell('pool_size'))            # cmp ebx,[pool_size]
    i_tz = alloc_arm.find(cmp_floor); i_cp = alloc_arm.find(cmp_psz)
    if i_tz < 0 or i_cp < 0: return False                         # M-nosizewrap drops the size reject -> REJECT
    if not (i_tz < i_cp): return False                            # cmp;jb (floor) before cmp;ja (ceiling)
    if alloc_arm[i_cp+len(cmp_psz):i_cp+len(cmp_psz)+2] != bytes([0x0F,0x87]): return False  # cmp ; ja
    if not target_is_oom(i_tz+3): return False                   # the jb (0F 82 at i_tz+3) must reach la_oom
    if not target_is_oom(i_cp+len(cmp_psz)): return False        # the ja (0F 87) must reach la_oom
    # (3) the first-fit search loop: cmp ecx,[nchunks] ; jae la_oom (the jae reaches OOM, not a dead fall-through).
    ff = bytes([0x3B,0x0D])+le(cell('nchunks'))+bytes([0x0F,0x83])   # cmp ecx,[nchunks] ; jae rel32
    i_ff = alloc_arm.find(ff)
    if i_ff < 0: return False
    if not target_is_oom(i_ff+len(ff)-2): return False           # the jae must reach la_oom
    # (3b) the CHUNK-TABLE-FULL guard before the split: cmp eax,LARDER_MAXCHUNKS ; jae la_oom (the jae reaches OOM, so an
    # at-cap alloc is rejected before emit_larder_split can overrun chunk[MAX]). M-nomaxchunks drops it -> the guard is
    # absent -> REJECT. (LARDER_MAXCHUNKS=16 fits imm8 -> cmp eax,imm8 = 83 F8 10.)
    mc = bytes([0xA1])+le(cell('nchunks'))+bytes([0x83,0xF8,LARDER_MAXCHUNKS,0x0F,0x83])  # mov eax,[nchunks] ; cmp eax,MAX ; jae
    i_mc = alloc_arm.find(mc)
    if i_mc < 0: return False                                     # M-nomaxchunks -> the table-full guard is gone -> REJECT
    if not target_is_oom(i_mc+len(mc)-2): return False           # the jae must reach la_oom
    # (4) the split-remainder insert EXACTLY ONCE (std ; rep movsd ; cld).
    if alloc_arm.count(bytes([0xFD,0xF3,0xA5,0xFC])) != 1: return False   # M-nosplit -> 0 -> REJECT
    # (5) the kernel-emit path: outi(0xE0) in do_alloc + the 0xE1..0xE2 dump frame in do_dump.
    if bytes([0xB0,0xE0,0xE6,OUT]) not in alloc_arm: return False
    if bytes([0xB0,0xE1,0xE6,OUT]) not in dump_arm: return False
    if bytes([0xB0,0xE2,0xE6,OUT]) not in dump_arm: return False
    # (6) do_free EXACT base match (NOT range) -- M-nointeriorfree replaces it.
    exact = bytes([0x8B,0x06,0x3B,0x05])+le(cell('al_ptr'))+bytes([0x0F,0x85])   # mov eax,[esi] ; cmp eax,[al_ptr] ; jne
    if exact not in free_arm: return False                        # M-nointeriorfree (range match) -> REJECT
    # (7) the prev+next coalesce deletes EXACTLY TWICE (cld ; rep movsd).
    if free_arm.count(bytes([0xFC,0xF3,0xA5])) != 2: return False # M-nocoalesce->0, M-noprevmerge/M-nonextmerge->1 -> REJECT
    return True


def assert_growheap(kelf, npages=GROWHEAP_NPAGES):
    """White-box structural co-pin for growheap's MULTI-PAGE heap (the LATE-BOUND output grade in growheap_latebound.py
       is the PRIMARY binding; this pins the multi-page structure + the cross-page coalesce path that an output forge
       could try to fake). ADDITIVE on assert_larder: the genuine growheap alloc/free/dump arms are byte-identical to
       larder, so assert_larder MUST still pass on the growheap kernel. Pins, beyond assert_larder:
         (G1) MULTI-PAGE pool: the pool_size init immediate AND the chunk[0].len init immediate == npages*PGSZ (> one
              page). M-singlepage caps both to PGSZ -> the genuine immediate is ABSENT -> REJECT.
         (G2) the FIXED, contiguous npages-page pool region really EXISTS: pool_base = GROWHEAP_POOL_VADDR, the region is
              reserved (excl hi const = pool_base+npages*PGSZ), and EACH of the npages pages is flipped identity+User
              (PTE[pool_base+i*PGSZ] <- (pool_base+i*PGSZ)|7) so the multi-page span is genuinely CPL3-writable.
         (G3) CROSS-PAGE coalesce: the do_free coalesce arms merge UNCONDITIONALLY across the page boundary -- the free
              arm does NOT contain a page-boundary refuse guard (`and eax, PGSZ-1`, the M-nocrosspagecoalesce inject).
              A within-page-only coalescer (M-nocrosspagecoalesce) carries that guard -> REJECT. (The guard's absence is
              an arm-LOCAL co-pin; the OUTPUT grade -- E OOMs where genuine fits the straddling hole -- is PRIMARY.)
       Returns True/False. Mirrors assert_larder / assert_cairn / assert_durability."""
    if not assert_larder(kelf): return False                      # the inherited heap structure must hold (additivity)
    le = lambda v: struct.pack('<I', v & 0xFFFFFFFF)
    cap = npages*GROWHEAP_PGSZ
    # (G1) MULTI-PAGE: pool_size + chunk[0].len init immediates == npages*PGSZ (NOT one page). M-singlepage -> REJECT.
    if (bytes([0xC7,0x05])+le(cell('pool_size'))+le(cap)) not in kelf: return False
    if (bytes([0xC7,0x05])+le(chunk_base()+4)+le(cap)) not in kelf: return False
    # (G2) the FIXED pool region: pool_base override + excl reservation + npages identity+User PTE flips.
    _,_,labels = build_elf(npages=npages); pt = labels['pt']
    if (bytes([0xB8])+le(GROWHEAP_POOL_VADDR)) not in kelf: return False           # pool_base = the fixed region base
    if (bytes([0xB8])+le(GROWHEAP_POOL_VADDR+cap)) not in kelf: return False       # excl hi = pool_base+cap (region reserved)
    for i in range(npages):
        pg = GROWHEAP_POOL_VADDR + i*GROWHEAP_PGSZ
        if (bytes([0xC7,0x05])+le(pt+(pg>>12)*4)+le(pg|7)) not in kelf: return False   # PTE[pool page i] <- identity+User
    # (G3) CROSS-PAGE coalesce: the free arm must NOT carry the page-boundary refuse guard. Re-locate the free arm.
    lk = bytes([0xB8])+le(KDATA)+bytes([0x8E,0xD8,0x8E,0xC0,0x8E,0xE0,0x8E,0xE8])   # load_kdata
    free_anchor = lk + bytes([0x89,0x0D])+le(cell('al_ptr'))                        # do_free prologue
    dump_anchor = lk + bytes([0xB0,0xE1,0xE6,OUT])                                  # do_dump prologue
    sf = kelf.find(free_anchor)
    if sf < 0: return False
    sd = kelf.find(dump_anchor, sf)
    if sd < 0: return False
    free_arm = kelf[sf:sd]
    if (bytes([0x25])+le(GROWHEAP_PGSZ-1)) in free_arm: return False   # `and eax, PGSZ-1` = the M-nocrosspagecoalesce refuse guard -> REJECT
    return True


def assert_delete(kelf, npages=GROWHEAP_NPAGES):
    """White-box co-pin for the mutable-FS DELETE arm (link42). The 3-BOOT reboot differential in delete_latebound.py is
       the PRIMARY binding; this pins the DEL arm's structure AND the output-INVISIBLE confused-deputy surface a forge
       could fake on the benign run (the DF/carry guards do not change the benign output but a hostile module exploits
       their absence). ADDITIVE on assert_cairn/assert_growheap (which must STILL pass on the DELETE kernel). Pins:
         (D1) the SYS_FS_DEL dispatch entry exists: cmp eax,12 ; je (83 F8 0C 0F 84).
         (D2) the DEL arm's access_ok CARRY guard: add edx,16 (83 C2 10) IMMEDIATELY followed by jb (0F 82). M-nocarrycheck
              drops the jb -> REJECT (output-invisible on the benign run; a hostile near-4GiB name leaks kernel memory).
         (D3) the DEL name-compare cld-adjacency: EXACTLY ONE `FC F3 A6` (cld;repe cmpsb) WITHIN the DEL arm. M-fsnocld -> 0;
              M-positional removes the compare entirely -> 0 -> REJECT.
         (D4) the FIXED-D dir scan bound: cmp ecx,FS_D (83 F9 08) within the DEL arm.
         (D5) the in-place TOMBSTONE `mov dword[esi],0` (C7 06 00 00 00 00) within the DEL arm. M-noop drops it; M-wipeall
              uses a `mov dword[edi],0` (C7 07) loop instead -> the C7 06 form is ABSENT -> REJECT.
         (D6) the dir WRITE-BACK + CACHE FLUSH within the DEL arm, IN ORDER: WRITE SECTORS (66 BA F7 01 B0 30 EE) then
              CACHE FLUSH (66 BA F7 01 B0 E7 EE). M-noflush drops the flush -> REJECT.
       Returns True/False. Mirrors assert_cairn / assert_growheap / assert_durability."""
    le = lambda v: struct.pack('<I', v & 0xFFFFFFFF)
    # (D1) the dispatch entry.
    disp = bytes([0x83,0xF8,SYS_FS_DEL,0x0F,0x84])
    if disp not in kelf: return False
    # Locate the DEL arm by its UNIQUE prologue: `mov [fs_nameptr],ebx` (89 1D <nameptr>) IMMEDIATELY followed by
    # `mov eax,[cur]` (8B 05 <cur>). PUT/GET interpose `mov [fs_payptr],ecx` (89 0D ...) there, so this 10-byte sequence
    # occurs ONLY at do_fs_del (a forge that drops the minimal prologue loses the access_ok entirely).
    del_anchor = bytes([0x89,0x1D])+le(cell('fs_nameptr'))+bytes([0x8B,0x05])+le(cell('cur'))
    ds = kelf.find(del_anchor)
    if ds < 0: return False
    # (D7) REACHABILITY (pin_reachability_not_presence): the dispatch `cmp eax,12 ; je rel32` must TARGET the live arm, not
    # dead bytes. do_fs_del begins with load_kdata (13 bytes: B8 <KDATA:4> 8E D8 8E C0 8E E0 8E E8) THEN the del_anchor, so
    # the label file-offset = ds-13. The je is `0F 84 rel32` at p+3 (after `83 F8 0C`), next instr at p+9, so the branch
    # target file-offset = p+9 + signed(rel32). File offsets map linearly to code-region vaddrs, so this equals the vaddr
    # arithmetic. Require the dispatch to actually jump to do_fs_del (a forge leaving the arm unreachable -> REJECT).
    matched_reach = False; p = -1
    while True:
        p = kelf.find(disp, p+1)
        if p < 0: break
        rel = int.from_bytes(kelf[p+5:p+9], 'little', signed=True)
        if p + 9 + rel == ds - 13: matched_reach = True; break
    if not matched_reach: return False
    # Bound the arm END at the gdtr+idtr descriptor-register pair that immediately follows: gdtr = `2F 00 <gdt:4>` then
    # idtr = `87 01 <idt:4>`. Find `2F 00` whose +6..+8 == `87 01` (robust to mutant byte-shifts, addresses unpinned).
    de = -1; p = ds
    while True:
        p = kelf.find(b'\x2f\x00', p+1)
        if p < 0: break
        if kelf[p+6:p+8] == b'\x87\x01': de = p; break
    if de < 0: return False
    arm = kelf[ds:de]
    # (D2) the access_ok carry guard: add edx,16 then jb.
    i = arm.find(bytes([0x83,0xC2,FS_NAMELEN]))
    if i < 0: return False
    if arm[i+3:i+5] != bytes([0x0F,0x82]): return False
    # (D3) the FULL-16-byte name compare, cld-led: `mov ecx,16 ; cld ; repe cmpsb` (B9 10 00 00 00 FC F3 A6) CONTIGUOUS,
    # exactly once. Binds (a) the compare length is 16 (a last-byte-only forge `mov ecx,1` -> REJECT), (b) the cld
    # (M-fsnocld drops FC -> REJECT), (c) it is a repe cmpsb -- the contiguous form cannot be satisfied by scattered dead bytes.
    cmp16 = bytes([0xB9,FS_NAMELEN,0x00,0x00,0x00, 0xFC, 0xF3,0xA6])
    if arm.count(cmp16) != 1: return False
    # (D4) the fixed-D scan bound.
    if bytes([0x83,0xF9,FS_D]) not in arm: return False
    # (D5) the in-place TOMBSTONE, BOUND to the matched-slot dataflow: `dec eax ; imul esi,eax,28 ; add esi,<dirbuf imm32>`
    # (48 6B F0 1C 81 C6 ?? ?? ?? ??) IMMEDIATELY followed (at +10) by `mov dword[esi],0` (C7 06 00 00 00 00). Proves the
    # tombstone clears VALID (offset 0) of the MATCHED slot (esi = dirbuf + (match-1)*28). M-noop drops the block; M-wipeall
    # uses a C7 07 loop; M-corruptlen uses C7 46 04 (clears LEN not VALID -- OUTPUT-INVISIBLE, this is its ONLY catcher) ->
    # each makes arm[q+10:q+16] != C7 06 00 00 00 00 -> REJECT. A dead C7 06 elsewhere cannot satisfy the recompute prefix.
    recompute = bytes([0x48, 0x6B,0xF0,FS_ENTSZ, 0x81,0xC6])
    # UNIQUE (count==1): `imul esi,eax,28` (6B F0 1C) is the matched-slot recompute; the scan uses `imul esi,ecx,28`
    # (6B F1 1C, a DIFFERENT byte), so this prefix occurs exactly once on the genuine path. Requiring exactly one defeats a
    # dead-bytes SPOOF (a forge inserting a second correct-looking recompute+tombstone block before the live wrong one to
    # satisfy a presence check -- a cross-model/Codex probe motivated this). M-wipeall (6B F9 1C) -> 0 occurrences -> REJECT.
    if arm.count(recompute) != 1: return False
    q = arm.find(recompute)
    # The recompute's imm32 (the `add esi,<imm32>` operand at q+6..q+10) MUST be the dirbuf base address, so esi =
    # dirbuf + (match-1)*28 = the MATCHED slot base, and the C7 06 clears its OFFSET 0 = VALID. Pinning the imm32 closes a
    # cross-model (Codex) forge: `add esi,dirbuf+4` (or +24) + `mov dword[esi],0` clears LEN (or data_lba) instead of VALID
    # -- OUTPUT-INVISIBLE (GET then emits nothing) and the byte-pin's ONLY-catcher otherwise. For the output-invisible
    # "absence by corruption" class the white-box pin MUST bind, not lean on the byte-pin.
    _,_,labels = build_elf(npages=npages, fsdel=True)
    if arm[q+6:q+10] != le(labels['dirbuf']): return False
    if arm[q+10:q+16] != bytes([0xC7,0x06,0x00,0x00,0x00,0x00]): return False
    # (D6) the dir write-back WRITE SECTORS then CACHE FLUSH, in order.
    wcmd  = bytes([0x66,0xBA,0xF7,0x01,0xB0,0x30,0xEE])
    flush = bytes([0x66,0xBA,0xF7,0x01,0xB0,0xE7,0xEE])
    iw = arm.find(wcmd)
    ifl = arm.find(flush, iw+1) if iw >= 0 else -1
    if iw < 0 or ifl < 0 or not (iw < ifl): return False
    return True


def assert_backfill(kelf, npages=GROWHEAP_NPAGES):
    """White-box co-pin for backfill's FIRST-FREE-SLOT PUT allocator (link 43, pays LEDGER D22). The 4-boot
       PUT-after-DELETE reuse differential + the host RAW-DIR/RAW-SECTOR ground-truth oracle (backfill_latebound.py)
       are the PRIMARY binding; this co-pins that SYS_FS_PUT allocates by FIRST-FREE-SLOT (the lowest valid==0 hole),
       NOT append-by-count, so a PUT after a NON-TAIL delete reuses the hole. ADDITIVE on assert_cairn / assert_delete
       / assert_growheap (each must STILL pass on the backfill kernel). Pins, within the do_fs_put arm:
         (R1) the first-free scan break: `cmp dword[esi+0],1 ; jne fs_put_found` (83 3E 01 0F 85) -- valid!=1 => the
              slot is FREE. M-wrongscan inverts it to `je` (0F 84, picks the first VALID slot -> overwrites a survivor).
         (R2) the slot index comes from the SCAN INDEX ecx: `mov [fs_nent],ecx` (89 0D <fs_nent>) is present and the
              append-by-count store `mov [fs_nent],eax` (A3 <fs_nent>) is ABSENT. M-append/M-tailappend (and the frozen
              delete PUT) use the count store -> the 89 0D form is absent / the A3 form is present -> REJECT.
         (R3) data_lba = FS_DATA_LO + freeslot, derived from the scan index: `mov eax,ecx ; add eax,FS_DATA_LO ;
              mov [fs_lba],eax` (89 C8 05 <FS_DATA_LO> A3 <fs_lba>) contiguous. M-decoupled fixes data_lba at
              FS_DATA_LO via `mov [fs_lba],FS_DATA_LO` (C7 05 ...) -> this contiguous sig is absent -> REJECT.
       Returns True/False. Mirrors assert_delete / assert_cairn."""
    le = lambda v: struct.pack('<I', v & 0xFFFFFFFF)
    # locate the do_fs_put arm by its 3-save prologue; the GET arm has the same prologue shape -> bounds the arm end.
    prol = bytes([0x89,0x1D])+le(cell('fs_nameptr'))+bytes([0x89,0x0D])+le(cell('fs_payptr'))+bytes([0x89,0x15])+le(cell('fs_len'))
    ps = kelf.find(prol)
    if ps < 0: return False
    ge = kelf.find(prol, ps+1)
    if ge < 0: return False
    arm = kelf[ps:ge]
    # (R0) the scan starts at slot 0: `xor ecx,ecx ; cmp ecx,FS_D` (31 C9 83 F9 08) contiguous. A scan-from-1 forge uses
    # `mov ecx,1` (B9 01 00 00 00) or a cursor-load -> this contiguous sequence is absent -> REJECT (the cross-model leg
    # found that a scan-from-1 allocator otherwise keeps the visible first-free signatures).
    if bytes([0x31,0xC9,0x83,0xF9,FS_D]) not in arm: return False
    # (R1) the first-free break is a JNE (valid!=1 => free), not a JE.
    if bytes([0x83,0x3E,0x01,0x0F,0x85]) not in arm: return False
    if bytes([0x83,0x3E,0x01,0x0F,0x84]) in arm: return False         # je form => M-wrongscan
    # (R2) slot from the scan index (89 0D fs_nent) present; the append-by-count store (A3 fs_nent) absent.
    if (bytes([0x89,0x0D])+le(cell('fs_nent'))) not in arm: return False
    if (bytes([0xA3])+le(cell('fs_nent'))) in arm: return False
    # (R3) data_lba = FS_DATA_LO + freeslot, contiguous (defeats M-decoupled's fixed FS_DATA_LO).
    if (bytes([0x89,0xC8,0x05])+le(FS_DATA_LO)+bytes([0xA3])+le(cell('fs_lba'))) not in arm: return False
    return True


def assert_highwater(kelf, npages=GROWHEAP_NPAGES):
    """White-box co-pin for link45's runtime free-frame allocator (the B1 byte-pin to build_elf(highwater=True) + the
       tri-substrate author-unknown-(-m) witness are the PRIMARY bindings; this pins the load-bearing structure so a
       white-box forge that tries to pass is caught, and provides the differential: the frozen tract kernel FAILS it).
       ADDITIVE on every prior assert (which must STILL pass on the highwater kernel). Pins, within the do_falloc /
       do_hwdump arms (located by label offset):
         (H1) the SYS_FALLOC + SYS_HWDUMP dispatch entries: cmp eax,13/14 ; je. (Frozen tract lacks them -> the differential.)
         (H2) the TOP-DOWN descending cursor: al_cur <- region_hi (A1<region_hi> A3<al_cur>) AND eax = al_cur-0x1000
              (A1<al_cur> 2D <0x1000>). M-hwbumpup (bottom-up from a baked 0x400000) lacks both -> REJECT.
         (H3) the RUNTIME PTE[V] install from the TOP-DOWN frame: A1<al_ptr> A3<tmp0> (or eax,7) A3<pt+(V>>12)*4>. The map
              reads the runtime top-down frame (al_ptr), not a baked frame. M-hwsingleframe maps a baked F_FRAME
              (B8<F_FRAME> instead of A1<al_ptr>) -> REJECT.
         (H4) the targeted invlpg [V] in do_falloc (0F 01 3D <V_VADDR>). M-hwnoinvlpg drops it -> REJECT.
         (H5) the kernel-emit reads the runtime frame: B0 F0 E6 E9 A1<al_ptr>. M-hwbakedaddr emits a baked addr
              (B8<imm>) -> the A1<al_ptr> after the marker is absent -> REJECT.
         (H6) do_hwdump: the re-map PTE[V] (A3<pt+V>), the targeted invlpg [V], the readback mov eax,[V] (A1<V_VADDR>),
              and the 0xF1/0xF3 frame markers -- the distinctness witness path.
         (H7) REACHABILITY (pin_reachability_not_presence): the SYS_FALLOC dispatch je must TARGET do_falloc."""
    le = lambda v: struct.pack('<I', v & 0xFFFFFFFF)
    # (H1) dispatch entries
    if bytes([0x83,0xF8,SYS_FALLOC,0x0F,0x84]) not in kelf: return False
    if bytes([0x83,0xF8,SYS_HWDUMP,0x0F,0x84]) not in kelf: return False
    # locate the arms by label offset (the genuine build's labels; the B1 byte-pin guarantees the kelf IS this build)
    _,_,labels = build_elf(npages=npages, fsdel=True, fsreuse=True, varsize=True, highwater=True)
    for need in ('do_falloc','do_hwdump','timer_handler','pt'):
        if need not in labels: return False
    foff = lambda v: 4096 + (v - LOAD)
    fa = foff(labels['do_falloc']); hd = foff(labels['do_hwdump']); tm = foff(labels['timer_handler'])
    if not (fa < hd < tm): return False
    falloc = kelf[fa:hd]; hwdump = kelf[hd:tm]
    pteV    = bytes([0xA3]) + le(labels['pt'] + (V_VADDR>>12)*4)        # mov [PTE_V], eax  (runtime frame)
    invlpgV = bytes([0x0F,0x01,0x3D]) + le(V_VADDR)                     # invlpg [V] (targeted)
    # (H2) top-down descending cursor
    if (bytes([0xA1])+le(cell('region_hi'))+bytes([0xA3])+le(cell('al_cur'))) not in falloc: return False
    if (bytes([0xA1])+le(cell('al_cur'))+bytes([0x2D])+le(0x1000)) not in falloc: return False
    # (H2b) the HARD per-boot cap: mov eax,[al_i] ; cmp eax,HW_MAXFRAMES ; jae (bounds the array index + the descent). M-hwnocap drops it.
    if (bytes([0x8B,0x05])+le(cell('al_i'))+bytes([0x3D])+le(HW_MAXFRAMES)+bytes([0x0F,0x83])) not in falloc: return False
    # (H3) runtime PTE[V] install from al_ptr (the top-down frame), not a baked frame
    runtime_install = bytes([0xA1])+le(cell('al_ptr'))+bytes([0xA3])+le(cell('tmp0'))+bytes([0x83,0xC8,0x07])+pteV
    if runtime_install not in falloc: return False
    # (H4) targeted invlpg in do_falloc
    if invlpgV not in falloc: return False
    # (H5) kernel-emit reads the runtime frame (al_ptr), not a baked address
    if (bytes([0xB0,HW_AMARK,0xE6,OUT,0xA1])+le(cell('al_ptr'))) not in falloc: return False
    # (H6) do_hwdump distinctness path: re-map + invlpg + readback + frame markers
    if pteV not in hwdump: return False
    if invlpgV not in hwdump: return False
    if (bytes([0xA1])+le(V_VADDR)) not in hwdump: return False          # mov eax,[V]  (readback)
    if bytes([0xB0,HW_DBEGIN,0xE6,OUT]) not in hwdump: return False
    if bytes([0xB0,HW_DEND,0xE6,OUT]) not in hwdump: return False
    # (H7) reachability: the SYS_FALLOC dispatch je targets do_falloc
    disp = bytes([0x83,0xF8,SYS_FALLOC,0x0F,0x84]); p = -1; reach = False
    while True:
        p = kelf.find(disp, p+1)
        if p < 0: break
        rel = int.from_bytes(kelf[p+5:p+9], 'little', signed=True)
        if p + 9 + rel == fa: reach = True; break
    return reach


def assert_varsize(kelf):
    """White-box co-pin for tract's MULTI-SECTOR variable-size PUT/GET (link 44). The 4-boot multi-sector forcing + the
       host raw ground-truth oracle (tract_latebound.py) are the PRIMARY binding; this co-pins the EMITTER MECHANISM so an
       honest-looking alternate emitter that passes the OUTPUT oracle but is NOT genuinely first-fit-by-LBA / ceil-sized /
       window-bounded / copy-min FAILS (the cross-model Codex design red-team flagged best-fit / slot-order / overcopy /
       no-window-bound forges). Pins, in the do_fs_put arm: (V1) ceil run-sizing `add eax,511 ; shr eax,9`; (V2) the
       free-map mark `mov byte[eax+fsused],1` (the recompute-from-dir map); (V3) the LBA-relative ridx `sub eax,FS_DATA_LO`
       (coalesce/mark BY LBA value, not slot order); (V4) the first-fit free-run check `cmp byte[eax+fsused],0 ; jne`
       (scan from the LOWEST sector -> first-fit, defeats best/worst-fit); (V5) the copy-min `cmp eax,512 ; jbe` (the
       multi-sector write loop copies min(512,remaining) -> partial-last-sector exact, no overcopy). In the do_fs_get arm:
       ceil; the run-window straddle guard `cmp runend,TRACT_DATA_HI ; ja reject`; and the copy-min (multi-sector read).
       The FROZEN backfill kernel (single-sector, no ceil/free-map/multi-sector) FAILS this -> the link differential.
       Locates the two FS arms by the shared 3-save prologue, exactly like assert_backfill. Returns True/False."""
    le = lambda v: struct.pack('<I', v & 0xFFFFFFFF)
    _, _, labels = build_elf(varsize=True); fsused = labels['fsused']
    prol = bytes([0x89,0x1D])+le(cell('fs_nameptr'))+bytes([0x89,0x0D])+le(cell('fs_payptr'))+bytes([0x89,0x15])+le(cell('fs_len'))
    ps = kelf.find(prol)
    if ps < 0: return False
    ge = kelf.find(prol, ps+1)
    if ge < 0: return False
    put = kelf[ps:ge]; get = kelf[ge:]
    ceil_sig  = bytes([0x05])+le(511)+bytes([0xC1,0xE8,0x09])         # V1: add eax,511 ; shr eax,9
    mark_sig  = bytes([0xC6,0x80])+le(fsused)+bytes([0x01])           # V2: mov byte[eax+fsused],1
    lba_sig   = bytes([0x2D])+le(FS_DATA_LO)                          # V3: sub eax,FS_DATA_LO (by-LBA)
    ff_sig    = bytes([0x80,0xB8])+le(fsused)+bytes([0x00,0x0F,0x85]) # V4: cmp byte[eax+fsused],0 ; jne (first-fit)
    chunk_sig = bytes([0x3D,0x00,0x02,0x00,0x00,0x0F,0x86])           # V5: cmp eax,512 ; jbe (copy min(512,rem))
    carry_sig = bytes([0x03,0x05])+le(cell('al_size'))+bytes([0x0F,0x82])   # V6: add eax,[al_size](runend) ; JC reject (OVERFLOW-SAFE run-window guard; Codex caught data_lba+need wrap)
    win_sig   = bytes([0x3D])+le(TRACT_DATA_HI)+bytes([0x0F,0x87])    # cmp runend,TRACT_DATA_HI ; ja reject (straddle)
    for sig in (ceil_sig, mark_sig, lba_sig, ff_sig, chunk_sig):      # PUT arm
        if sig not in put: return False
    for sig in (ceil_sig, carry_sig, win_sig, chunk_sig):            # GET arm (incl the overflow-safe carry-reject)
        if sig not in get: return False
    return True


# ============================ link42 (DELETE) baked prober + grade (STEP-0 silicon smoke) ============================
def module_fs_deleter(name16):
    # BOOT-2 (proc0, K=1, fresh boot): SYS_FS_DEL(name16) -> tombstone the named record IN PLACE + flush, then SYS_EXIT.
    # (The late-bound version -- name fed over COM1 -- lives in delete_latebound.py for the real gate.)
    m=Asm()
    nbytes = _push_bytes(m, name16)              # push the 16-byte name (esp -> name)
    m.raw(0x89,0xE3)                             # mov ebx,esp                  (name_ptr)
    m.raw(0xB8); m.blob(le32(SYS_FS_DEL))        # mov eax,12 (SYS_FS_DEL)
    m.raw(0xCD,0x30)                             # int 0x30 -> tombstone + flush
    m.raw(0x81,0xC4); m.blob(le32(nbytes))       # add esp (restore)
    m.raw(0xB3,0x00); m.raw(0xB8,0x01,0x00,0x00,0x00); m.raw(0xCD,0x30)   # SYS_EXIT(0)
    m.raw(0xEB,0xFE)
    return m.assemble()[0]

def grade_fs_found(stream, kend_elf, want_found):
    """DELETE grade via the FOUND flag (the STRONG deleted check). A found-probe getter SYS_FS_GETs a name and emits the
       1-byte found flag (eax). found==0 means the slot's valid==0 (REALLY tombstoned -- the scan skips valid!=1); found==1
       means the slot is still valid. So grading "deleted" as found==0 makes the whole "absence by corruption" class
       OUTPUT-VISIBLE: M-corruptlen (valid==1, len==0 -> GET found==1) and a tombstone-then-untombstone forge (valid
       restored to 1) both emit found==1 -> RED, where a payload-empty check (grade_fs_deleted) would read them as deleted.
       (A cross-model Codex leg drove this: the zero-length GET payload is not sufficient evidence of deletion; the VALID
       field must be observed directly.) GREEN iff the single emitted 1-byte frame == want_found (0 = deleted / 1 = present)."""
    errs=[]
    r=parse_head(stream)
    if not r:
        return ['no OWN table parsed (the kernel faulted before the found-probe dumped) -> RED']
    tail=r['_tail']
    wfs=[w for w in _wframes(tail) if w['closed'] and w['cs']==UCODE3 and (w['cs']&3)==3]
    if len(wfs)!=1 or len(wfs[0]['body'])!=1:
        errs.append(f'found-probe emitted {len(wfs)} frame(s) (body lens {[len(w["body"]) for w in wfs]}) != 1 frame of 1 byte (the probe faulted or GET was not handled)')
        return errs
    got=wfs[0]['body'][0]
    if got != want_found:
        kind = 'deleted (valid==0)' if want_found==0 else 'present (valid==1)'
        errs.append(f'FOUND={got} != want {want_found} -- the named record is NOT {kind}. found==1 with an empty payload is '
                    f'the "absence by corruption" forge (the slot is still valid, GET just returns len==0); found==0 is a '
                    f'genuine tombstone (the scan skips valid!=1).')
    if r['nprocs']!=1: errs.append(f'nprocs={r["nprocs"]} != 1 (delete is a single-program probe)')
    return errs
...
def grade_fs_deleted(stream, kend_elf):
    ...
    if nonempty:
        errs.append(f'NOT DELETED: BOOT-3 GET of the deleted name emitted {nonempty[0]["body"]!r} (len '
                    f'{len(nonempty[0]["body"])}) -- the record still resolved across the reboot. M-noop (no tombstone), '
                    f'M-positional (deleted the wrong slot), or M-noflush (tombstone lost on the reboot).')
    if r['nprocs']!=1: errs.append(f'nprocs={r["nprocs"]} != 1 (delete is a single-program probe)')
    return errs
...
if __name__=='__main__':
    cmd=sys.argv[1]
    if cmd=='backfillkernel':  # out [mut]  (build the BACKFILL kernel ELF: fsdel=True, fsreuse=True, npages=GROWHEAP_NPAGES; print kend)
        mut=sys.argv[3] if len(sys.argv)>3 and sys.argv[3]!='none' else None
        img,kend,_=build_elf(mut=mut, npages=GROWHEAP_NPAGES, fsdel=True, fsreuse=True); open(sys.argv[2],'wb').write(img); print('%x'%kend); sys.exit(0)
    elif cmd=='assertbackfill': # kelf -> exit 0 if the BACKFILL first-free-slot white-box co-pin holds
        sys.exit(0 if assert_backfill(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='tractkernel':   # out [mut]  (build the TRACT kernel ELF: npages=GROWHEAP_NPAGES, fsdel=True, varsize=True; print kend)
        mut=sys.argv[3] if len(sys.argv)>3 and sys.argv[3]!='none' else None
        img,kend,_=build_elf(mut=mut, npages=GROWHEAP_NPAGES, fsdel=True, varsize=True); open(sys.argv[2],'wb').write(img); print('%x'%kend); sys.exit(0)
    elif cmd=='assertvarsize': # kelf -> exit 0 if the TRACT multi-sector varsize white-box co-pin holds
        sys.exit(0 if assert_varsize(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='deletekernel':  # out [mut]  (build the DELETE kernel ELF: fsdel=True, npages=GROWHEAP_NPAGES; print kend -- the BACKFILL differential's no-reuse baseline)
        mut=sys.argv[3] if len(sys.argv)>3 and sys.argv[3]!='none' else None
        img,kend,_=build_elf(mut=mut, npages=GROWHEAP_NPAGES, fsdel=True); open(sys.argv[2],'wb').write(img); print('%x'%kend); sys.exit(0)
    elif cmd=='frozenkernel':  # out [mut]  (build the FROZEN baseline = growheap, fsdel=False; the differential's no-DEL kernel)
        mut=sys.argv[3] if len(sys.argv)>3 and sys.argv[3]!='none' else None
        img,kend,_=build_elf(mut=mut, npages=GROWHEAP_NPAGES, fsdel=False); open(sys.argv[2],'wb').write(img); print('%x'%kend); sys.exit(0)
    elif cmd=='assertdelete':  # kelf -> exit 0 if the DELETE white-box co-pin holds
        sys.exit(0 if assert_delete(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='fsdeleter':     # out name  (BOOT-2 baked deleter: SYS_FS_DEL <name>)
        open(sys.argv[2],'wb').write(module_fs_deleter(fs_name(sys.argv[3]))); sys.exit(0)
    elif cmd=='gradedeleted':  # stream kend  (GREEN iff the BOOT-3 GET of the deleted name emitted NOTHING -> deleted)
    ...
    elif cmd=='assertlarder':    # kelf -> exit 0 if the larder white-box co-pin holds
        sys.exit(0 if assert_larder(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='assertgrowheap':   # kelf -> exit 0 if the growheap (multi-page) white-box co-pin holds
        sys.exit(0 if assert_growheap(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='growheapkernel':   # out [mut]  (build the GROWHEAP kernel ELF -- npages=GROWHEAP_NPAGES -- print kend)
        mut=sys.argv[3] if len(sys.argv)>3 and sys.argv[3]!='none' else None
        img,kend,_=build_elf(mut=mut, npages=GROWHEAP_NPAGES); open(sys.argv[2],'wb').write(img); print('%x'%kend); sys.exit(0)
    elif cmd=='growheapwindow':   # print GROWHEAP_NPAGES GROWHEAP_PGSZ GROWHEAP_CAP GROWHEAP_POOL_VADDR
        print(GROWHEAP_NPAGES, GROWHEAP_PGSZ, GROWHEAP_CAP, GROWHEAP_POOL_VADDR); sys.exit(0)
    elif cmd=='fswriter':      # out  (BOOT-1: PUT ALPHA(target) then BRAVO(decoy))
        ...
        if errs: print('RED'); [print('  -',e) for e in errs]; sys.exit(1)
        print('GREEN'); sys.exit(0)
    elif cmd=='asserthighwater':   # kelf -> exit 0 if the link45 free-frame-allocator white-box co-pin holds
        sys.exit(0 if assert_highwater(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='assertvarsize':     # kelf -> exit 0 if tract's varsize co-pin still holds (additivity)
        sys.exit(0 if assert_varsize(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='assertdelete':      # kelf -> exit 0 if delete's co-pin still holds (additivity)
        sys.exit(0 if assert_delete(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='assertlarder':      # kelf -> exit 0 if larder's co-pin still holds (additivity)
        sys.exit(0 if assert_larder(open(sys.argv[2],'rb').read()) else 1)
