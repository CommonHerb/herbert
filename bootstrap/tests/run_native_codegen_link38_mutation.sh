...
boot_answer() { # modfile byte -> sets OUT (e9 file) and ANSWER (hex of the emitted DE<x>AD byte, or empty)
    ...
    grep -q LISTENING "$W/feed.log" 2>/dev/null || { echo "FAIL: link38 harness failure -- feeder never reached LISTENING (socket/QEMU launch dead; NOT a mutation bite)" >&2; : > "$HVMARK"; kill "$fp" 2>/dev/null; wait "$fp" 2>/dev/null; return; }
    timeout 60 qemu-system-x86_64 -kernel "$KELF" -initrd "$mod" -debugcon file:"$OUT" \
        ...
        grep -qvE 'terminating on signal' "$OUT.qerr" 2>/dev/null && { echo "FAIL: link38 harness failure -- QEMU launch error: $(grep -vE 'terminating on signal' "$OUT.qerr" | head -1)" >&2; : > "$HVMARK"; }   # F2a: only a NON-timeout stderr line is a launch failure; a timeout-kill (hang bite) is left to the grader
    wait "$fp" 2>/dev/null
...
if [[ -e "$HVMARK" ]]; then echo "FAIL: link38 HARNESS FAILURE -- a feeder never reached LISTENING (dead socket/QEMU); fail-closed, NOT a genuine pass"; rm -f "$HVMARK"; exit 1; fi
