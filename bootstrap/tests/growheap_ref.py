#!/usr/bin/env python3
# growheap_ref.py -- reference builder + grader for "growheap" (native-codegen link 57 / kernel-arc link 41); this link's CONTRACT is bootstrap/tests/run_native_codegen_link57.sh (+ its _mutation.sh). HEADER ANCESTRY (blind-audit A3, 2026-08-29): this file was cloned from larder_ref.py and the descriptive block that follows is the ANCESTOR's -- kept as provenance, NOT as this file's spec. Ancestor header:
...
def build_code(kstack, kend, mut=None, stage='full', npages=1):
...
def build_elf(mut=None, stage='full', npages=1):   # cleave L847-859 + growheap npages (npages=1 == larder byte-for-byte)
    code0,_=build_code(0,0,mut,stage,npages); clen=len(code0)
    memsz=12+clen+16384
    kstack=LOAD+memsz; kend=LOAD+memsz
    code,labels=build_code(kstack,kend,mut,stage,npages); assert len(code)==clen,(len(code),clen)
...
if __name__=='__main__':
    cmd=sys.argv[1]
    if cmd=='assertlarder':    # kelf -> exit 0 if the larder white-box co-pin holds
