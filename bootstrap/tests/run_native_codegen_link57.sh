...
PY
    ...
    if echo "$bochs_emit" | grep -q '^GREEN'; then ok "(C-Bochs) the late-bound cross-page witness on the EMITTED kernel is GREEN on the 2nd substrate: the kernel-emitted trace == the host first-fit golden (the make-or-break alloc fits the cross-page-coalesced hole) on Bochs' chipset"
    else fail_test "(C-Bochs) Bochs cross-page witness not GREEN: $(echo "$bochs_emit" | tr '\n' ';')"; fi
else
