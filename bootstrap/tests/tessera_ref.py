...
# ---- tessera (shared-memory aliasing) constants ----
# The shared frame F and its two NON-IDENTITY virtual aliases. All three are page-aligned, within the 16 MiB identity
# map (so F is real present RAM), and high enough to clear the kernel image (~1 MiB) and the page tables. F != SH_VA,
# SH_VB (non-identity); SH_VA != SH_VB (aliasing observable). All three are RESERVED from the region bump allocator via
# fixed excl[] entries (see build_code), so a region can never be placed on them -- belt-and-suspenders, since the
# region march also stays far below 6 MiB by layout (verified: 8 procs each 1 committed page + an 8-page grow window
# top out ~0x153000 ~= 1.3 MiB, a >4x margin). HONEST-SCOPE RESIDUE: the windows are FIXED vaddrs; a Multiboot loader
# that placed a MODULE or mbinfo at exactly SH_VA/SH_VB/SH_FRAME would collide (the alias install overwrites that PTE) --
# this is the lodger-style audited-assertion residue (verified clear on QEMU-TCG/KVM/Bochs; dynamically choosing the
# window to dodge the loaded image needs the DEFERRED runtime frame allocator, not this minimal atom).
SH_VA   = 0x600000          # producer's window onto the shared frame  (vaddr; identity frame 0x600000 is ABANDONED)
SH_VB   = 0x800000          # consumer's window onto the shared frame  (vaddr; identity frame 0x800000 is ABANDONED)
SH_FRAME= 0xA00000          # the SHARED physical frame both windows alias  (!= SH_VA, != SH_VB -> NON-IDENTITY)
SH_N    = 4                 # number of seed-derived payload words (SH_N*4 = 16 bytes > tandem's 4-byte register mailbox)
SH_TAG  = 0x5E0000          # payload high bits: word_i = (SH_TAG | (seed & 0xFF)) + i*0x100  (distinct, seed-dependent)
SH_SENTINEL = 0x7E55E4A0    # the producer writes this at [SH_VA + SH_N*4] LAST; the consumer spin-waits on [SH_VB+SH_N*4]
SH_SEED = 0x5A              # default late-bound seed byte for the producer (fed over COM1; the kernel is seed-agnostic)
assert SH_FRAME != SH_VA and SH_FRAME != SH_VB and SH_VA != SH_VB, 'tessera windows must be non-identity + distinct'
def sh_words(seed):         # the SH_N seed-derived payload words the consumer must emit (the grade reproduces these)
    base=(SH_TAG | (seed & 0xFF)) & 0xFFFFFFFF
    return [(base + i*0x100) & 0xFFFFFFFF for i in range(SH_N)]
NEXCL=8+3*MAXPROC           # excl[] capacity: 8 fixed (kernel/mb/cm/elf/mmap + tessera SH_VA/SH_VB/SH_FRAME) + 2 per
...
def build_code(kstack, kend, mut=None, stage='full'):
    ...
    # tessera: reserve the shared window (SH_VA/SH_VB/SH_FRAME) so the region allocator can never place a region on it
    excl_push(lambda:(a.raw(0xB8),a.blob(le32(SH_VA))),    lambda:(a.raw(0xB8),a.blob(le32(SH_VA+0x1000))))      # SH_VA page
    excl_push(lambda:(a.raw(0xB8),a.blob(le32(SH_VB))),    lambda:(a.raw(0xB8),a.blob(le32(SH_VB+0x1000))))      # SH_VB page
    excl_push(lambda:(a.raw(0xB8),a.blob(le32(SH_FRAME))), lambda:(a.raw(0xB8),a.blob(le32(SH_FRAME+0x1000))))   # SH_FRAME page
    ...
    # ===== tessera: install the SHARED NON-IDENTITY frame (THE LINK'S CORE) =====
    # PTE[SH_VA>>12] <- SH_FRAME|7 ; PTE[SH_VB>>12] <- SH_FRAME|7  (present+RW+User). NON-IDENTITY (frame != vaddr) AND
    # ALIASED (both PTEs hold SH_FRAME). The first PTEs in the stack where PFN != VPN. M-identity installs IDENTITY
    # (Va->Va, Vb->Vb, still User): accessible but the frames are DISJOINT -> the consumer reads its own window, never
    # the producer's words (the homestead-M-eager analogue -- proves ALIASING, not the User permission, is load-bearing).
    # M-onlyone aliases only Va (Vb identity -> disjoint). M-noinstall skips both (window stays identity+Supervisor ->
    # a CPL3 access #PFs). M-supervisor maps both to F but Supervisor (|3) -> a CPL3 access #PFs. (The cr3 reload below
    # is defensive hygiene; an M-noflush mutation would be VACUOUS -- the windows are first touched AFTER the install,
    # so the TLB never cached the stale identity mapping -- and is therefore not in the mutation set.)
    if mut!='noinstall':
        va_frame = SH_VA if mut=='identity' else SH_FRAME
        vb_frame = SH_VB if mut in ('identity','onlyone') else SH_FRAME
        perm = 3 if mut=='supervisor' else 7                                     # M-supervisor: omit the User bit
        a.raw(0xC7,0x05); a.absR('pt',(SH_VA>>12)*4); a.blob(le32(va_frame|perm))   # PTE[SH_VA] <- frame | P|RW|(User)
        a.raw(0xC7,0x05); a.absR('pt',(SH_VB>>12)*4); a.blob(le32(vb_frame|perm))   # PTE[SH_VB] <- frame | P|RW|(User)
        a.raw(0xC7,0x05); a.blob(le32(SH_FRAME + SH_N*4)); a.blob(le32(0))       # zero the sentinel slot (via SH_FRAME id)
        a.raw(0xB8); a.absR('pd'); a.raw(0x0F,0x22,0xD8)                         # reload cr3 (TLB flush; lineage hygiene)
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'tessera', cd['nprocs'])
...
# ============================ tessera FORCING: the producer + consumer (shared-frame aliasing) ============================
def module_producer():
    # PRODUCER (proc0): read a late-bound seed byte (SYS_READ over COM1), derive SH_N seed-dependent words, write them
    # THROUGH SH_VA (its alias onto the shared frame), then write SH_SENTINEL at [SH_VA+SH_N*4] LAST so the consumer
    # never observes a half-filled frame (program-order stores on this uniprocessor PM target -- not a formal SMP/weak-
    # memory barrier). Emits NOTHING itself -- the words appear in output ONLY if the
    # consumer reads them through the OTHER alias (SH_VB). word_i = (SH_TAG | (seed & 0xFF)) + i*0x100.
    m=Asm()
    m.raw(0xB8,0x00,0x00,0x00,0x00)               # mov eax,0  (SYS_READ)
    m.raw(0xCD,0x30)                              # int 0x30 -> seed byte in eax
    m.raw(0x25); m.blob(le32(0xFF))               # and eax,0xFF
    m.raw(0x0D); m.blob(le32(SH_TAG))             # or  eax,SH_TAG  -> base word in eax (no int after here: eax survives)
    for i in range(SH_N):                         # write SH_N words THROUGH SH_VA (the producer's alias)
        if i>0: m.raw(0x05); m.blob(le32(0x100))  # add eax,0x100  (word_i = base + i*0x100)
        m.raw(0xA3); m.blob(le32(SH_VA + i*4))    # mov [SH_VA + i*4], eax   (absolute window write)
    m.raw(0xC7,0x05); m.blob(le32(SH_VA + SH_N*4)); m.blob(le32(SH_SENTINEL))   # sentinel LAST (uniproc program order)
    m.raw(0xB3,0x00); m.raw(0xB8,0x01,0x00,0x00,0x00); m.raw(0xCD,0x30)   # SYS_EXIT(0)
    m.raw(0xEB,0xFE)
    return m.assemble()[0]

def module_consumer():
    # CONSUMER (proc1): spin-read [SH_VB+SH_N*4] until it equals SH_SENTINEL (deterministic; the spin is preemptible so
    # it cannot starve the producer -- tickover's fairness), then read the SH_N words THROUGH SH_VB and SYS_WRITE each.
    # The words it emits could ONLY have come through the shared frame: its own region is peer-isolated, the 4-byte
    # register mailbox can't carry SH_N*4=16 bytes, and the payload is late-bound. Each word is re-read fresh from the
    # frame after the GP-clobbering int 0x30, so no register state must survive the syscall.
    m=Asm()
    m.lbl('spin')
    m.raw(0xA1); m.blob(le32(SH_VB + SH_N*4))     # mov eax,[SH_VB + SH_N*4]  (the sentinel slot, via SH_VB)
    m.raw(0x3D); m.blob(le32(SH_SENTINEL))        # cmp eax,SH_SENTINEL
    m.j(JNE,'spin')                               # not ready -> spin (preempted normally; producer runs, writes, exits)
    for i in range(SH_N):
        m.raw(0xA1); m.blob(le32(SH_VB + i*4))    # mov eax,[SH_VB + i*4]   (read word THROUGH SH_VB -- the alias)
        m.raw(0x50)                               # push eax  (onto the consumer's OWN in-region stack)
        m.raw(0x8D,0x0C,0x24)                     # lea ecx,[esp]   (buf ptr in-region -> SYS_WRITE access_ok passes)
        m.raw(0xBA,0x04,0x00,0x00,0x00)           # mov edx,4
        m.raw(0xB8,0x02,0x00,0x00,0x00)           # mov eax,2 (SYS_WRITE)
        m.raw(0xCD,0x30)
        m.raw(0x83,0xC4,0x04)                     # add esp,4
...
def assert_tessera(kelf):
    """White-box structural pin for tessera's NON-IDENTITY ALIAS install (the byte-pin to build_elf() is the PRIMARY
       binding; this is the structural co-pin codex's verdict demanded). Pins the two runtime PTE installs that no
       identity map / U-bit flip / fixed-large reserve can produce:
         PTE[SH_VA>>12] <- SH_FRAME|7   (C7 05 <pt+SH_VA_idx> <SH_FRAME|7>)
         PTE[SH_VB>>12] <- SH_FRAME|7   (C7 05 <pt+SH_VB_idx> <SH_FRAME|7>)
       Both installs carry the SAME frame SH_FRAME (-> ALIASED: PFN(Va)==PFN(Vb)) AND SH_FRAME differs from both
       vaddrs' page bases (-> NON-IDENTITY: PFN != VPN). M-identity installs SH_VA/SH_VB (identity) -> inst_va absent.
       M-onlyone installs SH_VB identity -> inst_vb absent. M-noinstall omits both. Returns True/False."""
    _,_,labels=build_elf()
    pt=labels['pt']
    inst_va = bytes([0xC7,0x05])+le32(pt+(SH_VA>>12)*4)+le32(SH_FRAME|7)
    inst_vb = bytes([0xC7,0x05])+le32(pt+(SH_VB>>12)*4)+le32(SH_FRAME|7)
    if inst_va not in kelf: return False                                # PTE[Va] <- SH_FRAME present?
    if inst_vb not in kelf: return False                                # PTE[Vb] <- SH_FRAME present?  (same frame -> aliased)
    if (SH_FRAME & 0xFFFFF000) == (SH_VA & 0xFFFFF000): return False     # NON-IDENTITY vs Va
    if (SH_FRAME & 0xFFFFF000) == (SH_VB & 0xFFFFF000): return False     # NON-IDENTITY vs Vb
    return True


def grade_tessera(stream, kend_elf, K=2, seed=SH_SEED):
    """tessera FULL grade -- SHARED MEMORY via a non-identity aliased frame. K=2: proc0=PRODUCER (writes SH_N seed-derived
       words THROUGH SH_VA + a sentinel; emits nothing), proc1=CONSUMER (spins on the sentinel, reads the SH_N words
       THROUGH SH_VB, SYS_WRITEs each). GREEN requires the consumer to emit ALL SH_N late-bound words exactly once -- it
       could only have read them through the shared frame. On the FROZEN furlough kernel (no alias install) SH_VA/SH_VB
       are identity+Supervisor: the producer #PFs writing the shared window (or, with a permission-only forge, the
       consumer reads a DISJOINT frame) -> the words never appear -> RED. Returns errs (CLI prints GREEN/RED)."""
    errs=[]
    r=parse_head(stream)
    if not r:
        return ['no OWN table parsed (kernel faulted before the procs ran, or the shared window #PFd before the dump) -> RED']
    tail=r['_tail']
    # Count each seed-word across ANY closed CPL3 4-byte write frame. No per-proc/eip-range attribution is needed here
    # (unlike grade()/grade_furlough): the producer module is hand-pinned and does NO SYS_WRITE (only the consumer emits),
    # and the modules are byte-fixed by the gate, so a "producer emits the words" forge is impossible. cnt!=1 RED-fails
    # both empty output (FROZEN/M-identity) and any double-emit.
    wfs=[w for w in _wframes(tail) if w['closed'] and w['ln']==4 and w['cs']==UCODE3 and (w['cs']&3)==3]
    bodies=[struct.unpack('<I',w['body'])[0] for w in wfs]
    want=sh_words(seed)
    for i,wd in enumerate(want):
        cnt=bodies.count(wd)
        if cnt!=1:
            errs.append(f'consumer word[{i}]=0x{wd:08x} emitted {cnt} times != 1 '
                        f'(FROZEN/M-identity: the consumer reads a DISJOINT frame, never the producer payload; '
                        f'M-noinstall: the shared window #PFs)')
    if r['nprocs']!=K: errs.append(f'nprocs={r["nprocs"]} != K={K}')
    return errs
...
if __name__=='__main__':
    ...
    elif cmd=='modproducer':   # out  (tessera proc0: writes SH_N seed-derived words THROUGH SH_VA + sentinel; emits nothing)
        open(sys.argv[2],'wb').write(module_producer())
    elif cmd=='modconsumer':   # out  (tessera proc1: spins on the sentinel, reads SH_N words THROUGH SH_VB, SYS_WRITEs each)
        open(sys.argv[2],'wb').write(module_consumer())
    elif cmd=='gradetess':     # stream kend [K] [seed]  (tessera shared-frame grade)
        stream=open(sys.argv[2],'rb').read(); kend=int(sys.argv[3],16)
        K=int(sys.argv[4]) if len(sys.argv)>4 else 2
        seed=int(sys.argv[5],0) if len(sys.argv)>5 else SH_SEED
        errs=grade_tessera(stream,kend,K,seed)
        if errs: print('RED'); [print('  -',e) for e in errs]; sys.exit(1)
        print('GREEN'); sys.exit(0)
    elif cmd=='tessera':
        sys.exit(0 if assert_tessera(open(sys.argv[2],'rb').read()) else 1)
    elif cmd=='ncells': print(len(CELLS))
    else: raise SystemExit('usage: kernelelf|kend|modtiny|modspinner|modworker|modworker_ten|modgrower|modreader|modpeer|modproducer|modconsumer|modhostile|modhostileread|grade|gradehead|gradeten|gradehome|gradefurl|gradetess|gradehostile|rollcall|tenement|homestead|furlough|tessera|ncells')
