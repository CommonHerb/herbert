...
def build_code(kstack, kend, mut=None, stage='full'):
    ...
    a.lbl('mbok')
    a.raw(0x40)                                        # larder: inc eax -> tmp0 = min(MSLOTS,nprocs) + 1 (the +1 pool page)
    ...
    ld('eax',arr('alloc_lo'),'ecx'); mme(cell('pool_base'))          # pool_base = alloc_lo[nprocs]  (the extra page)
    mmi(cell('pool_size'), LARDER_POOL)                              # pool_size = N
    mmi(cell('nchunks'), 1)                                          # one chunk to start
    mem(cell('pool_base')); a.raw(0xA3); a.blob(le32(chunk_base()))  # chunk[0].base = pool_base
    a.raw(0xC7,0x05); a.blob(le32(chunk_base()+4)); a.blob(le32(LARDER_POOL))   # chunk[0].len = N
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'larder', cd['nprocs'])
