...
boot() { # modfile kind -> sets OUT (debugcon stream)
    ...
    grep -q LISTENING "$W/feed.log" 2>/dev/null || { echo "FAIL: link42 harness failure -- feeder never reached LISTENING (socket/QEMU launch dead; NOT a mutation bite)" >&2; : > "$HVMARK"; kill "$fp" 2>/dev/null; wait "$fp" 2>/dev/null; return; }
    timeout 90 qemu-system-x86_64 -kernel "$REFK" -initrd "$mod" -debugcon file:"$OUT" \
        ...
        grep -qvE 'terminating on signal' "$OUT.qerr" 2>/dev/null && { echo "FAIL: link42 harness failure -- QEMU launch error: $(grep -vE 'terminating on signal' "$OUT.qerr" | head -1)" >&2; : > "$HVMARK"; }   # F2a: only a NON-timeout stderr line is a launch failure; a timeout-kill (hang bite) is left to the grader
    wait "$fp" 2>/dev/null
}
...
if [[ -e "$HVMARK" ]]; then echo "FAIL: link42 HARNESS FAILURE -- a feeder never reached LISTENING (dead socket/QEMU); fail-closed, NOT a genuine pass"; rm -f "$HVMARK"; exit 1; fi
