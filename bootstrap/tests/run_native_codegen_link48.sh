...
bochs_run() { # e9out seed
    ...
    local bfp=$!; local i; for i in $(seq 1 50); do grep -q LISTENING "$d/feed.log" 2>/dev/null && break; done
...
if have_bochs; then
    ...
    else fail_test "(C) Bochs N=$N -> $(python3 "$REF" gradehome "$work/b" "$KEND" "$N" "$SEED" 2>&1 | tr '\n' ';')"; fi
else
