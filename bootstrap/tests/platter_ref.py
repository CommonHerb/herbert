#!/usr/bin/env python3
# platter_ref.py -- reference builder + grader for "platter" (native-codegen link 53 / kernel-arc link 37); this link's CONTRACT is bootstrap/tests/run_native_codegen_link53.sh (+ its _mutation.sh). HEADER ANCESTRY (blind-audit A3, 2026-08-29): this file was cloned from lethe_ref.py and the descriptive block that follows is the ANCESTOR's -- kept as provenance, NOT as this file's spec. Ancestor header:
...
SYS_DISK_READ=5            # link37 NEW (unused eax in lethe): ATA PIO LBA28 single-sector read, access_ok on the LBA
...
def build_code(kstack, kend, mut=None, stage='full'):
    ...
    a.raw(0x83,0xF8,SYS_DISK_READ); a.j(JE,'do_disk_read') # eax==5 -> DISK READ  (link37 NEW)
...
def parse_head(stream):
    ...
    try:
        cd['_tail']=debugcon.FramedTail(stream[i:], 'platter', cd['nprocs'])
...
if __name__=='__main__':
    cmd=sys.argv[1]
    if cmd=='diskprober':      # out  (link37 K-hop disk pointer-chase prober)
